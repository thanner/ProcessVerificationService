 ...
 * <p>Requires at least two boolean input operands to be evaluated. 
 */
public class HLAndOperator extends HLExpressionOperator {
	
	/**
	 * Default Constructor.
	 */
	public HLAndOperator() {
		minNumberInputs = 2;
	}
		...
		if (getExpressionNode().getChildCount() == 0) {
			...
			for (int i=0; i<getExpressionNode().getChildCount(); i++) {
				...
				if (result == "") {
					...
					result = result + " AND " + "(" + childExpr.evaluateToString() + ")";
				}
			}
			return "(" + result + ")";
		}
