 ...
 * <p>Per default assigns equal probability to all nominal values.
...
public class HLNominalDistribution implements HLAttributeValue {
	...
	/**
	 * Specifies the possible values for the data attribute based on 
	 * the given list. 
	 * <br>
	...
	public void setPossibleValues(List<String> values) {
		...
		for (String current : values) {
			if (possibleValues.containsKey(current) == false) {
				...
				possibleValues.put(current, new Integer(possibleValues.get(current) + 1));
			}	
		}
	}
	
	/**
	 * Directly sets the possible values for this data attribute together 
	 * with their relative frequencies. 
	 * @param values the new nominal distribution of values
	 */
	...
	/** 
	 * Adds the given value to the set of possible values for this attribute.
	 * <br>
	 * Will increase the frequency of the value if already there
	 * @param aValue the nominal value to be added
	 */
	public void addPossibleValueNominal(String aValue) {
		if (possibleValues.containsKey(aValue) == false) {
			...
			possibleValues.put(aValue, new Integer(possibleValues.get(aValue) + 1));
		}	
	}
	
	/**
	 * Retrieves the frequency value for the given value.
	 * <br>
	 * Note that in relation to the frequencies of the other nominal 
	 * values, a percentage, or likelihood, for the given value can be 
	 * represented.
	 * @param value the nominal value for which the frequency is requested
	...
	/**
	 * Calculates the sum of all frequencies observed for all
	 * possible values in this distribution. 
	...
	/**
	 * Specifies the frequency for the given value.
	 * <br>
	 * Note that in relation to the frequencies of the other nominal 
	 * values, a percentage, or likelihood, for the given value can be 
	 * represented.
	 * <br>
	 * If the given nominal value is not among the possible nominal values, 
	 * this method has no effect. 
	 * @param value the nominal value for which the frequency should be recorded
	 * @param freq the frequency for the given nominal value
	 */
	...
	public Object clone() {
		...
		try {
			...
			o.possibleValues = (HashMap<String, Integer>) possibleValues.clone();
			return o;
