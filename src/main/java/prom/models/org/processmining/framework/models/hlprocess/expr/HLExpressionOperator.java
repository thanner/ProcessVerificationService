 ...
 * <p>To be valid, it needs to have at least as many inputs as specified (and potentially 
 * a maximum number of inputs, if specified). 
 * An operator is not allowed to be a leaf node in the expression tree.
 */
public abstract class HLExpressionOperator extends HLExpressionElement {
	
	/*
	 * (non-Javadoc)
	 * @see org.processmining.framework.models.hlprocess.expr.HLDataExpressionElement#hasMinNumberInputs()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.framework.models.hlprocess.expr.HLExpressionElement#evaluate()
	 */
	public HLExpressionOperand evaluate() {
		...
		for (int i=0; i<node.getChildCount(); i++) {
			...
			HLExpressionElement exprEl = (HLExpressionElement) childNode.getUserObject();
	 ...
	 * @param operands the input operands to be evaluated
	 * @return the output operand (potentially to be further evaluated)
	 */
	protected abstract HLExpressionOperand evaluateOperands(List<HLExpressionOperand> operands);
