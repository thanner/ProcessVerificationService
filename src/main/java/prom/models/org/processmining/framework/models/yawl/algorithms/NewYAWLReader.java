...
/**
 * YAWL parser/reader that parses newYAWL and creates a hlYAWL data structure rather than a low-level YAWL control-flow
 * model.
 *
 * @author Anne Rozinat
 */
public class NewYAWLReader {
	...
	public static HLYAWL read(InputStream input) throws Exception {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equals("decomposition")) {
				...
				// make the root net of the YAWL model the name of the whole model
		...
		result.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.DATA_INITIAL_VAL);
		result.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.DATA_AT_TASKS);
		result.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.ROLES_AT_TASKS);
		result.getHLProcess().getGlobalInfo().addPerspective(HLTypes.Perspective.CHOICE_DATA);
	 ...
	 * @param model The YAWL model being created
	 * @param isRootNet indicates whether decomposition is root net (beware: even if passed as <code>false</code>
	 * it can still be detected to be <code>true</code> in attribute)
	 */
	...
	public static void parseTask(Node rootNode, YAWLDecomposition decomposition) {
		...
		// TODO Anne: check how to deal with this "unknown:normal" event type as this will be
		// displayed as part of the task name - not desired. YAWL models will contain atomic tasks,
		// which might be decomposed into sub-processes etc., but the offering, start, completion etc.
		// is on a different abstraction layer (happens in the engine and becomes logged etc.)
	...
	// TODO Anne: check whether can be parsed during first reading process already as
	// unnecessarily duplicates code. but the HLYAWL model is created based on the parsed
	// control-flow model. therefore, so far, plain import code is exactly the same (only less)
	// as in the YAWLReader -- put them together where possible

	/**
	 * Add high-level information (such as data and resources) to imported YAWL model.
	 * @param rootNode the given node
	 * @param result the resulting HLYAWL model to be created
	 * @param isRootNet indicates whether decomposition is root net (beware: even if passed as <code>false</code>
	 * it can still be detected to be <code>true</code> in attribute)
	 */
	public static void parseDecompositionForHLInformation(Node rootNode, HLYAWL result, boolean isRootNet) {
		NodeList nodes = rootNode.getChildNodes();
		// Store attributes
		Node isRootNetNode = rootNode.getAttributes().getNamedItem("isRootNet");
		// Root net status could also be indicated in the decomposition attribute
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			} else if (isRootNet == true && node.getNodeName().equals("inputParam")) {
				parseDataVariable(node, result);
			} else if (isRootNet == true && node.getNodeName().equals("outputParam")) {
				parseDataVariable(node, result);
			}

			// Parse output parameters output parameters at decompositions for data perspective
			// for non-root net decompositions only as root is no task in process
			else if (isRootNet == false && node.getNodeName().equals("outputParam")) {
				...
				if (parent.getNodeName().equals("decomposition")) {
					...
					if (fromNode != null) {
						...
						for (HLActivity act : result.getHLProcess().getActivities()) {
							...
							// matches activity names that start like the composition
							// but then are followed by an underscore and some number
							if (actName.matches(decompositionName + "_([0-9])*")) {
								addDataAttributeToHLActivity(node, result, act);
							}
						}
					} else {
						Message.add("The id of the decomposition could not be read when parsing " +
								"for output parameters.", Message.ERROR);
					}
				}
			}
	 ...
	 * @param decomposition The given decomposition
	 */
	public static void parseFlowsIntoForHLInformation(Node rootNode, HLYAWL result) {
		...
		if (fromNode != null) {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (node.getNodeName().equals("flowsInto")) {
					...
					parseFlowTargets(toNode, predicateNode, isDefaultFlowNode, orderingNode, result, hlAct);
				}
			}
		}
	}

	public static void parseFlowTargets(Node toNode, Node predicateNode, Node isDefaultFlowNode, Node orderingNode,
			HLYAWL result, HLActivity hlAct) {
		// get the target node
		String toElementName = toNode.getNodeValue();
		HLActivity targetAct;
		// add a new condition target for flow element, if exists
		if ((predicateNode != null || isDefaultFlowNode != null) && hlAct != null) {
			ModelGraphVertex yawlTask = result.findModelGraphVertexForActivity(hlAct.getID());
			HLChoice choice = result.findChoice(yawlTask);
			// add target to choice
			if (choice != null) {

				// (a) try whether target is already activity
				targetAct = result.getHLProcess().findActivityByName(toElementName);
				if (targetAct != null) {
					HLCondition cond = choice.addChoiceTarget(targetAct.getID());
					writeDataExpression(predicateNode, cond, result, orderingNode);
				} else {
					// (b) try to read condition node and find targets from there
					for (ModelGraphVertex graphNode : result.getGraphNodes()) {
						if (graphNode.getIdentifier().equals(toElementName)) {
							// look for out transitions of this place (as place cannot be a target)
	 ...
	 * <p>If the predicate node is <code>null</code> it is assumed that this will be a
	 * 'default' (i.e., "else") node.
	 * @param predicateNode the predicate node holding the XPATH expression
	 * @param cond the condition where this expression should be set
	 */
	public static void writeDataExpression(Node predicateNode, HLCondition cond, HLYAWL result,
			Node orderingNode) {
		if (predicateNode != null) {
			String expressionString = predicateNode.getNodeValue();
			expressionString = expressionString.trim();
			expressionString = expressionString.replaceAll("number\\([/A-Za-z]*/(.*)/text\\(\\)\\)", "$1");
			expressionString = expressionString.replaceAll("boolean\\([/A-Za-z]*/(.*)/text\\(\\)\\)", "$1");
			expressionString = expressionString.replaceAll("[/A-Za-z]*/(.*)/text\\(\\)", "$1");
			...
			if (orderingNode != null) {
				try {
					long numericValue = Long.parseLong(orderingNode.getNodeValue());
					cond.setExpression(new HLDataExpression(operator, (int)numericValue));
	 ...
	 * @param operands the oparands to be parsed
	 * @param operator the operator to which the operands should be added
	 * @param result the high level process for accessing attribute information
	 */
	public static void parseExpressionOperands(String[] operands, HLExpressionOperator operator, HLYAWL result) {
		for (int i=0; i<operands.length; i++) {
			...
			try {
				...
				HLAttributeValue hlValue = new HLNumericValue((int)numericValue);
			...
			HLAttribute hlAtt = result.getHLProcess().findAttributeByName(operand);
			if (hlAtt != null) {
				HLAttributeOperand hlOp = new HLAttributeOperand(hlAtt.getID(), result.getHLProcess());
				operator.addSubElement(hlOp);
			// (c) is boolean value
	...
	/**
	 * Parses the name of the output data element and assign to corresponding task.
	 * @param varNode the "ouputParam" node
	 * @param result high level yawl model to be filled
	 * @param act the high level activity that provides this attribute
	 */
	public static void addDataAttributeToHLActivity(Node varNode, HLYAWL result,
			HLActivity act) {
	 ...
	 * @param varNode the "localVariable" node
	 * @param result the HLYawl model to be filled
	 */
	public static void parseDataVariable(Node varNode, HLYAWL result) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			} else if (node.getNodeName().equals("initialValue") || node.getNodeName().equals("defaultValue")) {
				initialValue = node.getTextContent();
			}
		}
		// read type
		if (type.equals("boolean")) {
			HLBooleanAttribute boolAtt = new HLBooleanAttribute(name, result.getHLProcess());
		...
		} else if (type.endsWith("double") || type.equals("long") || type.equals("duration")) {
			HLNumericAttribute numAtt = new HLNumericAttribute(name, result.getHLProcess());
			if (initialValue != null) {
				// TODO: check whether next to int also other types could be supported!
				...
				if (parsedInitVal != null) {
					...
					Message.add("Initial or default value for attribute " + name +
							" could not be read. ", Message.DEBUG);
					// link the initial value to the distribution of possible values
					// i.e., make it random
		...
		// read all remaining types as nominal attributes
			...
			HLNominalAttribute nomAtt = new HLNominalAttribute(name, result.getHLProcess());
			if (initialValue != null) {
				// read the provided initial value
...
//			}
	 ...
	 * @param node the "resourcing" node
	 * @param result the HLYawl model to be filled
	 */
					...
					if (subSubNode.getNodeName().equals("initialSet")) {
						...
						for (int k = 0; k < roleNodes.getLength(); k++) {
							...
							if (roleNode.getNodeName().equals("role")) {
								...
								// ignore Nobody role tag as default in hlActivity is "nobody"
								...
								if (roleName.equals("nobody") == false) {
									...
									// all groups with equal name will be mapped on the same group
									Iterator<HLGroup> groupIt = hlProc.getGroups().iterator();
									while (groupIt.hasNext()) {
										HLGroup grp = groupIt.next();
										if (grp.getID().toString().equals(roleName)) {
											belongingHlGroup = grp;
										}
									}
									// if role read for the first time -> make new group
									if (belongingHlGroup == null) {
										HLID grpID = new HLID(roleName);
										belongingHlGroup = new HLGroup("Role ID: " + roleName, hlProc, grpID);
									}
									...
									while (actIt.hasNext()) {
										...
										if (act.getName().equals(taskName)) {
											act.setGroup(belongingHlGroup.getID());
										}
									}
								}
							}
	...
	/**
	 * TODO Anne: actually relates to the data element that is involved in expression.
	 * check and remove
	 *
	 * Parses the "completedMappings" element within the "task" tag.
	 * @param node the "completedMappings" node
	 * @param result the HLYawl model to be filled
	 */
//	public static void parseDataMapping(Node node, HLYAWL result, String taskName) {
//		NodeList subNodes = node.getChildNodes();
//		Node subNode;
//		for (int i = 0; i < subNodes.getLength(); i++) {
//			subNode = subNodes.item(i);
