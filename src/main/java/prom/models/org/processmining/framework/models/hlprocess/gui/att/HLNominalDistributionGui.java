...
/**
 * Represents the GUI of a nominal distribution. 
 * Allows to specify the relative frequencies of all possible values  
 * and to add and remove values.
 */
public class HLNominalDistributionGui extends HLAttributeValueGui {
	 ...
	 */
	public HLNominalDistributionGui(HLNominalDistribution dist, HLAttributeGui parent) {
		super(parent);
		if (dist != null) {
	...
	public JPanel getPanel() {
		if (panel == null) {
			...
			label.setToolTipText("<HTML>Specifies the relative frequency of the nominal values<br>" + 
	...
	public HLNominalDistribution getValue() {
		HLNominalDistribution result = (HLNominalDistribution) parent.getHLAttribute().getPossibleValues().clone();
