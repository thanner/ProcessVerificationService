...
public abstract class HLExpressionElement implements Cloneable {
	 ...
	 * element. 
	 * <p>Subclasses are expected to initialize this value if it is meaningful
	 * and to indicate via {@link #hasMinNumberInputs()} whether it is meaningful
	 * or not. 
	 */
	 ...
	 * element.
	 * <p>Subclasses are expected to initialize this value if it is meaningful
	 * and to indicate via {@link #hasMaxNumberInputs()} whether it is meaningful
	 * or not. 
	 */
	...
	/**
	 * Returns the root node of this expression tree. The expression 
	 * element is associated to this node and cann be retrieved via
	 * {@link DefaultMutableTreeNode#getUserObject()}.
	 * <p>In contrast to the expression element itself, the node can be 
	 * asked for its child nodes. 
	 ...
	 * @param newNode the new node in the expression tree for this element
	 */
	 ...
	 * <p>Transparently creates a new tree node representing the expression element.
	 * @see #addSubExpression(DefaultMutableTreeNode)
	 * @param item the node to be added as a child
	 ...
	 * <p>Preserves existing sub tree structure but assumes that expression 
	 * elements are already properly associated to all expression nodes.
	 * @see #addSubElement(HLExpressionElement)
	 * @param subNode the sub expression tree node to be added
	 */
	...
	/**
	 * Retrieves the minimum number of input elements required for this expression
	 * element.
	 * <p>Use {@link #hasMinNumberInputs()} to find out whether a minimum number has
	 * been specified for this expression element. 
	 */
	...
	/**
	 * Retrieves the maximum number of input elements required for this expression
	 * element.
	 * <p>Use {@link #hasMaxNumberInputs()} to find out whether a minimum number has
	 * been specified for this expression element. 
	 */
	...
	/**
	 * Checks recursively whether the minumum and maximum number input requirements are
	 * satisfied (for this expression element and for each of its children). 
	 * <p>Note that no type checking is performed and, therefore, the expression 
	...
	public boolean isValid() {
		...
		if (hasMinNumberInputs() && node.getChildCount() < minNumberInputs) {
			return false;
		} else if (hasMaxNumberInputs() && node.getChildCount() > maxNumberInputs ) {
			return false;
		} 
		// now check child nodes
		for (int i=0; i<node.getChildCount(); i++) {
			DefaultMutableTreeNode child = (DefaultMutableTreeNode) node.getChildAt(i);
			HLExpressionElement childExpr = (HLExpressionElement) child.getUserObject();
	...
	/**
	 * <p>Evaluates the expression element based on its input parameters.
	 * <p>Assumes that the expression is valid and the operator can be applied 
	 * to its input operands. As a result, the sub tree connected to this operator
	...
	/**
	 * Subclasses need to indicate whether a minimum number of required input elements
	 * has been specified or not. 
	 * @return <code>true</code> if a minimum number of input elements is required, 
	 * <code>false</code> otherwise
	 */
	public abstract boolean hasMinNumberInputs();
	
	/**
	 * Subclasses need to indicate whether a maximum number of required input elements
	 * has been specified or not. 
	 * @return <code>true</code> if a maximum number of input elements is required, 
	 * <code>false</code> otherwise
	 */
	public abstract boolean hasMaxNumberInputs();

	/**
	 * Subclasses need to provide a string representation. 
	 * Operators should give the name (or symbol) or the operator while 
	 * operands should provide a suitable value representation. 
	 */
	public abstract String toString();
	
	/**
	 * Generates a string representation of the subtree starting 
	 * from this element. 
