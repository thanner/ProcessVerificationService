 ...
 * <p>For example, in the expression <code>Amount > 500</code> "Amount"
 * would be a placeholder referring to some attribute in the process 
 * (i.e., correspond to an attribute operand) while "500" would be a
 * value operand and ">" would be the operator linking the two. 
 * <p>This implies that to actually evaluate an expression containing
 * attribute placeholders, concrete values need to be available for these
 * attributes (rather than aggregate information such as value range etc.). 
 * 
 * @see HLValueOperand
 */
public class HLAttributeOperand extends HLExpressionOperand {
	...
	/**
	 * The attribue from the high-level process that this
	 * placeholder operand is referring to. 
	 */
	protected HLID attID;
	/**
	 * The high level process from which the actual attribute object
	 * can be obtained.
	 */
	 ...
	 * @param att the attribute that serves as a placeholder in the expression
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.framework.models.hlprocess.expr.HLExpressionOperand#getValue()
	 */
	...
	/**
	 * Replaces the current attribute ID by the new ID. 
	 * Only if the given oldID matches the current ID (no effect otherwise).
	 * @param oldID the ID that needs to match the current ID
	 * @param newID the new ID that should be assigned 
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.framework.models.hlprocess.expr.HLDataExpressionElement#toString()
	 */
