 ...
 */
public class HLSmallerEqualOperator extends HLExpressionOperator {

	/**
	 * Default constructor.
	 */
	public HLSmallerEqualOperator() {
		minNumberInputs = 2;
		maxNumberInputs = 2;
	}
	...
	protected HLExpressionOperand evaluateOperands(List<HLExpressionOperand> operands) {
		...
		if (firstValue instanceof HLNumericValue && secondValue instanceof HLNumericValue) {
			boolean result = ((HLNumericValue) firstValue).getValue() <= ((HLNumericValue) secondValue).getValue();
