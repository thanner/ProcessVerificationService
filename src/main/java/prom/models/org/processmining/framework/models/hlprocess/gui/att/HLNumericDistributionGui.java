...
/**
 * Represents the GUI of a numeric data attribute, i.e., the possible values correspond 
 * to some numeric random distribution. 
 */
public class HLNumericDistributionGui extends HLAttributeValueGui {
	...
	/**
	 * Default Constructor.
	 * <p>Note that the given numeric distribution is assumed to be an HLGeneralDistribution.
	 * @param parent the numeric distribution to be edited through this GUI
	 */
	public HLNumericDistributionGui(HLNumericDistribution dist, HLAttributeGui parent) {
		super(parent);
		possibleValuesNumeric = HLDistributionGuiManager.getDistributionGui(dist.getValue(), this);
	}
	...
	public JPanel getPanel() {
		...
		label.setToolTipText("Specifies the distribution of the numeric values");
	...
	public HLNumericDistribution getValue() {
		return new HLNumericDistribution(possibleValuesNumeric.getDistribution());
	}
