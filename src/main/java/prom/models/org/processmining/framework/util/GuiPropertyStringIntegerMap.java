...
public class GuiPropertyStringIntegerMap implements TableModelListener, ListSelectionListener {
	...
	private GuiPropertyStringIntegerMap propertyReference = this; // hack: need reference in anonymous class def
	// GUI attributes
	 ...
	 */
	public GuiPropertyStringIntegerMap(String name, HashMap<String, Integer> values) {
	 ...
	 * @param values the initial string values of this property
	 */
	public GuiPropertyStringIntegerMap(String name, String description, HashMap<String, Integer> values) {
	 ...
	 */
	public GuiPropertyStringIntegerMap(String name, HashMap<String, Integer> values, GuiNotificationTarget target) {
	 ...
	 */
	public GuiPropertyStringIntegerMap(String name, String description, HashMap<String, Integer> values,
			GuiNotificationTarget target, boolean editable) {
	 ...
	 * @param editable whether the displayed table should be editable or not
	 */
	public GuiPropertyStringIntegerMap(String name, String description, HashMap<String, Integer> values,
			GuiNotificationTarget target, int height, boolean editable) {
		myName = name;
	...
	public void tableChanged(TableModelEvent e) {
        ...
        if (column == 0) {
        	// first remove the old value as the possible value string has changed
        	myValues.remove(model.getLastEditedString());
        } 
        myValues.put((String) model.getValueAt(row, 0), (Integer) model.getValueAt(row, 1));
    }
	...
	public JPanel getPropertyPanel() {
		if (resultPanel == null) {
			...
			// TODO: check whether Java 6 can be assumed here (feature only in Java 6)
			// TODO: check whether second one works with selection and removal (simpler)
			...
			if (myEditable == true) {
				...
				myRemoveButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						int[] selectedRows = myJTable.getSelectedRows();
						StringIntegerTableModel model = (StringIntegerTableModel) myJTable.getModel();
						for (int i=0; i<selectedRows.length; i++) {
							// TODO: check whether Java 6 can be assumed here (feature only in Java 6)
							// translate selected rows into rows in the model (view may be resorted)
							//selectedRows[i] = myJTable.convertRowIndexToModel(selectedRows[i]);
							String selectedValue = (String) model.getValueAt(selectedRows[i], 0);
	...
	private class StringIntegerTableModel extends AbstractTableModel {
		...
		/**
		 * Returns the old value of the "Nominal Value" cell that was edited 
		 * the last time.
		...
		public void setValueAt(Object value, int row, int col) {
			if (value != null) {
				if (col == 0) {
					// first save old data value (needs to be replaced in listener)
					lastEditedString = (String) data[row][0];
					for (int i=0; i<data.length; i++) {
						// only allow update if new string is not yet in the set of possible values
