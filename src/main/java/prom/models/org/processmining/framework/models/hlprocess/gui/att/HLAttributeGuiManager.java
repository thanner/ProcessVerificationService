...
public class HLAttributeGuiManager {
	...
	/**
	 * Creates a new attribute value GUI based on the kind of passed value object.
	 * Changes on the GUI will be directly reported back to the underlying attribute.
	 * @param parent the attribute gui for which the GUI is requested
	 * @return the GUI of matching value type
	 */
	public static HLAttributeValueGui getAttributeValueGui(HLAttributeValue value, HLAttributeGui parent) {
		if (value instanceof HLBooleanDistribution) {
			return new HLBooleanDistributionGui((HLBooleanDistribution) value, parent);
		} else if (value instanceof HLNominalDistribution) {
			return new HLNominalDistributionGui((HLNominalDistribution) value, parent);
		} else if (value instanceof HLNumericDistribution) {
			return new HLNumericDistributionGui((HLNumericDistribution) value, parent);
