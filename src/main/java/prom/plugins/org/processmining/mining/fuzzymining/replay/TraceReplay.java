...
public class TraceReplay {
	...
	public TraceReplay(FuzzyGraph graph, LogReader log, int traceIndex) 
		...
		this.coverage = 0.0;
		this.matches = new MatchType[this.instance.getAuditTrailEntryList().size()];
	...
	public double replayTrace() throws IndexOutOfBoundsException, IOException {
		...
		while(lastNode == null) {
			...
			lastIndex = events.findLogEventNumber(lastAte.getElement(), lastAte.getType());
		...
		possibleFollowers.put(lastNode, new HashSet<Node>(followers.get(lastNode)));
		...
		for(int i=1; i<ateList.size(); i++) {
			...
			currentIndex = events.findLogEventNumber(currentAte.getElement(), currentAte.getType());
			...
			// analyze state transition
				...
				possibleFollowers.put(currentNode, new HashSet<Node>(followers.get(currentNode)));
				matches[i] = MatchType.VALID;
				countCorrect++;
			}
	...
	protected boolean checkNode(Node current, HashMap<Node,Set<Node>> possibleFollowers) {
