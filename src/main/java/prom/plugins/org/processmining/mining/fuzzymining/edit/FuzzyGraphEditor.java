...
public class FuzzyGraphEditor extends JPanel implements Provider,
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		if(modifiedGraph != null) {
			provided.add(new ProvidedObject("Modified Fuzzy Graph", new Object[] { modifiedGraph }));
		}
		if(originalGraph != null) {
			provided.add(new ProvidedObject("Original Fuzzy Graph", new Object[] { originalGraph }));
		}
			...
			for(ProvidedObject po : item.getProvidedObjects()) {
				for(Object o : po.getObjects()) {
					if(o instanceof MutableFuzzyGraph) {
						...
						modifiedGraph = (MutableFuzzyGraph)originalGraph.clone();
						break;
					}
				}
				if(modifiedGraph != null) {
	...
	public AnalysisInputItem[] getInputItems() {
		return new AnalysisInputItem[] {
			new AnalysisInputItem("Fuzzy Graph") {
