...
public class CaseAnimationPanel extends JComponent implements AnimationListener {
	...
	protected void paintComponent(Graphics g) {
		...
		ArcAnimationKeyframe keyframe = this.anim.getCaseAnimation().getKeyframe(time);
		for(TokenAnimation anim : keyframe.getTokenAnimations()) {
			drawFlag(g2d, fontMetrics, flagPaneHeight, anim, time, coolTime);
		}
		// draw shadings
		GradientPaint gradient = new GradientPaint(0, border, new Color(0, 0, 0, 160), 0, border + border, new Color(0, 0, 0, 0), false);
		g2d.setPaint(gradient);
		g2d.fill(new Rectangle2D.Float(0, 0, width + 1, border + border + 1));
		gradient = new GradientPaint(0, height - border - border, new Color(0, 0, 0, 0), 0, height - border, new Color(0, 0, 0, 160), false);
		g2d.setPaint(gradient);
		g2d.fill(new Rectangle2D.Float(0, height - border - border, width + 1, border + border + 1));
		g2d.setColor(new Color(0, 0, 0));
		g2d.fill(new Rectangle2D.Float(0, 0, width + 1, border));
		g2d.fill(new Rectangle2D.Float(0, height - border, width + 1, border + 1));
	}
	
	protected void drawFlag(Graphics2D g2d, FontMetrics fontMetrics, float height, TokenAnimation anim, long time, long coolTime) {
		// calculate flag coordinates
		float flagY = border + (height * anim.completedPercentage(time)) - flagHeight;
		...
		GradientPaint gradient = new GradientPaint(flagX, flagY, ColorSettings.setAlpha(flagBg, 0.4f),
				flagX, flagY + flagHeight, flagBg, false);
		g2d.setPaint(gradient);
		g2d.fill(flagPath);
		// set border color and draw flag border
		Color borderColor = ColorSettings.darken(flagBg, 0.5f);
		gradient = new GradientPaint(flagX, flagY, ColorSettings.setAlpha(borderColor, 0.5f),
				flagX, flagY + flagHeight, borderColor, false);
		g2d.setPaint(gradient);
		g2d.draw(flagPath);
		// draw instance name
		...
		g2d.drawString(name, flagX + halfFlagWidth - (float)(sBounds.getWidth() / 2f), 
				flagY + (flagHeight / 2f) + (float)(sBounds.getHeight() * 0.6f));
	}
	...
	protected float getOffset(String caseId) {
		...
		if(ofObj != null) {
			...
			currentOffset = currentOffset + 5 + (random.nextFloat() * flagWidth);
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.vis.anim.AnimationListener#started()
	 */
	public void started() {
		// ignore
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.vis.anim.AnimationListener#stopped()
	 */
	public void stopped() {
		// ignore
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.vis.anim.AnimationListener#updateModelTime(long)
	 */
