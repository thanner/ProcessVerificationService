...
public class BinaryMetric extends Metric {
	...
	public BinaryMetric(String aName, String aDescription, int aSize) {
		...
		size = aSize;
		// use sparse matrices on high event count (exponential matrix size, #cells * 8 bytes)
		if(size < 512) {
	...
	public double getMeasure(int fromIndex, int toIndex) {
		...
		if(normalized == null) {
			normalized = MatrixUtils.normalize(relations.copy(), normalizationMaximum);
		}
		// retrieve result
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.metrics.Metric#setNormalizationMaximum(double)
	 */
	...
	public boolean isValid() {
		return (normalizationMaximum > 0.0 && MatrixUtils.getMaxValue(relations) > 0.0);
	}
