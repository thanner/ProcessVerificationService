...
public class FuzzyNodeEditor extends JPanel {
	...
	public FuzzyNodeEditor(FuzzyGraphEditorPanel parent, GGNode node, ActionListener closeListener) {
		this.parent = parent;
		this.graphNode = node;
		this.fuzzyNode = (Node)node.getAttribute(GGAdapter.ATTR_FUZZY_GRAPH_ELEMENT);
	...
	protected void initializeGui() {
		...
		JPanel namePanel = FuzzyGraphEditorPanel.layoutHorizontally(new Component[] {nameLabel, Box.createHorizontalStrut(5), nameField}, 20);
		// node type
		...
		JPanel typePanel = FuzzyGraphEditorPanel.layoutHorizontally(new Component[] {typeLabel, Box.createHorizontalStrut(5), typeField}, 20);
		// significance slider
		significanceSlider = new NiceDoubleSlider("significance", 0, 1.0, fuzzyNode.getSignificance(), Orientation.VERTICAL);
		if(isCluster) {
		...
		deleteNodeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				((MutableFuzzyGraph)fuzzyNode.getGraph()).hidePermanently(fuzzyNode);
				parent.updateGraphLayout();
				closeListener.actionPerformed(new ActionEvent(this, 0, "node deleted"));
			}
		});
		// close button
		...
		if(isCluster == true) {
			...
			String warning = "<html>You can only edit the name of a cluster node, or " +
							...
							"nodes.</html>";
			JLabel warningLabel = FuzzyGraphEditorPanel.createMessageLabel(warning);
		...
		this.add(Box.createVerticalStrut(15));
		this.add(FuzzyGraphEditorPanel.centerHorizontally(deleteNodeButton, 25));
