...
public class ClusterUtils {
	...
	public static double calculateDiameter(ClusterNode cluster) {
		...
		for(int x=0; x<clusterNodeSet.length; x++) {
			for(int y=0; y<clusterNodeSet.length; y++) {
				if(clusterNodeSet[x].equals(clusterNodeSet[y])) { continue; }
				path = findPath(clusterNodeSet[x], clusterNodeSet[y], clusterNodeSet, clusterNodeSet.length - 1);
				if(path != null) {
	...
	protected static ArrayList<Node> findPath(Node a, Node b, Node[] nodeSet, int maxDepth) {
		...
		if(graph.getBinarySignificance(indexA, indexB) > 0.0) {
			...
			for(int i=0; i<nodeSet.length; i++) {
				if(nodeSet[i].getIndex() == indexA) { continue; }	// skip current source index
				if(graph.getBinarySignificance(indexA, nodeSet[i].getIndex()) > 0.0) {
					// found successor to node A
					currentPath = findPath(graph.getPrimitiveNode(nodeSet[i].getIndex()), b, nodeSet, maxDepth - 1);
					if(currentPath != null) {
	...
	protected static double getPathCorrelation(ArrayList<Node> path) {
		...
		for(int i=1; i<path.size(); i++) {
			post = path.get(i);
			curCor = graph.getBinaryCorrelation(pre.getIndex(), post.getIndex());
			if(curCor < minCor) {
