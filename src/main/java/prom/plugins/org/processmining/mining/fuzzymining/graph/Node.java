...
public class Node implements DotFileWriter {
	...
	public boolean isDirectlyConnectedTo(Node other) {
		if(other instanceof ClusterNode) {
			...
			return ((graph.getBinarySignificance(index, other.index) > 0.0) ||
					(graph.getBinarySignificance(other.index, index) > 0.0));
		}
	}
	...
	public String getToolTipText() {
		LogEvent evt = graph.getLogEvents().get(index);
		return "<html><table><tr colspan=\"2\"><td>" + evt.getModelElementName() + "</td></tr>"
			+ "<tr><td>Event type:</td><td>" + evt.getEventType() + "</td></tr>"
			+ "<tr><td>Significance:</td><td>" + MutableFuzzyGraph.format(getSignificance()) + "</td></tr>";
	}
