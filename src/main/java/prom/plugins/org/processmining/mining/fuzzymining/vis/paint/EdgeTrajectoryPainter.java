...
public class EdgeTrajectoryPainter extends Painter {
	...
	public void paint(long modelTime, Graphics2D g2d) {
		BasicStroke stroke = new BasicStroke(canvas.getLineWidth(0.6f), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
		g2d.setStroke(stroke);
		ArcAnimationKeyframe keyframe = null;
		GGCurve curve = null;
		long fadeTime = canvas.getAnimation().getFadeTime();
		List<ArcAnimation> arcAnimations = canvas.getAnimation().getArcAnimations();
	...
	protected void drawCurveSegment(Graphics2D g2d, long modelTime, long fadeTime, GGCurve curve, TokenAnimation anim) {
		long idleTime = modelTime - anim.getStart();
		// determine start color
		Color startColor;
		if(idleTime > fadeTime) {
			startColor = canvas.getColorSettings().getActiveEdgeTrajectoryStart();
		} else {
			float percentage = (float)idleTime / (float)fadeTime;
			startColor = ColorSettings.mix(
					canvas.getColorSettings().getActiveEdgeBackground(), 
					canvas.getColorSettings().getActiveEdgeTrajectoryStart(),
					percentage);
			percentage = 1f - (percentage * 0.5f);
		}
		// determine trajectory coordinates
		float[][] points = curve.getPartialPath(anim.completedPercentage(modelTime));
		...
		GradientPaint gradient = new GradientPaint(firstX, firstY, startColor, 
				lastX, lastY, canvas.getColorSettings().getActiveEdgeBackground(), false);
