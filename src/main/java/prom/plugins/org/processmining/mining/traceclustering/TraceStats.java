...
public class TraceStats {
	...
	public class SingleTraceStat {
		...
		protected void parseInstance() throws IndexOutOfBoundsException, IOException {
			...
			int lastEvent = logEvents.findLogEventNumber(lastAte.getElement(), lastAte.getType());
			events.add(logEvents.get(lastEvent));
			AuditTrailEntry currentAte;
			int currentEvent;
			for(int i=1; i<ateList.size(); i++) {
				currentAte = ateList.get(i);
				currentEvent = logEvents.findLogEventNumber(currentAte.getElement(), currentAte.getType());
				events.add(logEvents.get(currentEvent));
				followerMatrix.set(lastEvent, currentEvent, followerMatrix.get(lastEvent, currentEvent) + 1);
				lastAte = currentAte;
