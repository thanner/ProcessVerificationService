...
public class TaskAtomicPainter extends Painter {
	...
	public void paint(long modelTime, Graphics2D g2d) {
		BasicStroke stroke = new BasicStroke(canvas.getBorderWidth(), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
		...
		List<TaskAnimation> taskAnimations = canvas.getAnimation().getPrimitiveTaskAnimations();
		...
		for(TaskAnimation taskAnimation : taskAnimations) {
			...
			if(idleTime >= fadeTime) {
				...
				textColor = canvas.getColorSettings().getPassiveTaskForeground();
				borderColor = canvas.getColorSettings().getPassiveTaskBorder();
			} else {
				float percentage = (float)idleTime / (float)fadeTime;
				bgColor = ColorSettings.mix(
						canvas.getColorSettings().getActiveTaskBackground(), 
						canvas.getColorSettings().getPassiveTaskBackground(), 
						percentage);
				textColor = ColorSettings.mix(
						canvas.getColorSettings().getActiveTaskForeground(), 
						canvas.getColorSettings().getPassiveTaskForeground(), 
						percentage);
				borderColor = ColorSettings.mix(
						canvas.getColorSettings().getActiveTaskBorder(), 
						canvas.getColorSettings().getPassiveTaskBorder(), 
						percentage);
			}
			...
			// paint background
			GradientPaint gradient = new GradientPaint(x, y, bgColor, 
					x, y + height, ColorSettings.darken(bgColor, 0.8f), false);
			...
			float vSlack = height - (float)nameBounds.getHeight() - (float)typeBounds.getHeight();
			...
			// paint border
			gradient = new GradientPaint(x, y, borderColor, 
					x, y + height, ColorSettings.fade(borderColor, 0.7f), false);
			g2d.setPaint(gradient);
			g2d.draw(rect);
		}
	}
