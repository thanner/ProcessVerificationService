...
public class FuzzyModelPanel extends GradientPanel implements Provider {
	...
	public FuzzyModelPanel(MetricsRepository metrics, int numberOfNodes) {
		this(new MutableFuzzyGraph(metrics), metrics.getLogReader(), metrics, numberOfNodes);
	}
	
	public FuzzyModelPanel(FuzzyGraph graph, LogReader log, MetricsRepository metrics, int numberOfNodes) {
	...
	protected void setupUI() {
		...
		FlatTabbedPane tabPane = new FlatTabbedPane("", new Color(240, 240, 240, 230), 
				new Color(100, 100, 100), new Color(220, 220, 220, 150));
		fastTransformerPanel = new FastTransformerPanel(graph, log, numberOfNodes);
		...
		if(metrics != null) {
			...
			unaryMetricsPanel = 
					new UnaryMetricsPanel(metrics.getLogReader().getLogSummary().getLogEvents());
			...
			binaryMetricsComparisonPanel = new BinaryMetricsComparisonPanel(metrics);
			// add to tabbed pane
	...
	public ProvidedObject[] getProvidedObjects() {
		...
		objects[ftpo.length] = new ProvidedObject("Original Fuzzy Model", new Object[] { originalGraph });
		return objects;
	}
