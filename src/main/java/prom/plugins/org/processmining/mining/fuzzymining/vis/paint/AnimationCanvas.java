...
public class AnimationCanvas extends JComponent implements AnimationListener {
	...
	public AnimationCanvas(Animation animation, ColorSettings settings) {
		...
		this.boxHeight = animation.getPrimitiveTaskAnimations().get(0).getHeight();
	...
	public void setZoom(float zoom) {
		this.zoom = zoom;
		leftBorder = boxHeight * animation.getParser().getBoundsHeight() * SCALE_FACTOR * zoom;
		topBorder = leftBorder / 2;
		bottomBorder = topBorder;
		rightBorder = leftBorder * 3;
		this.width = animation.getParser().getBoundsWidth() * SCALE_FACTOR * zoom;
		this.height = animation.getParser().getBoundsHeight() * SCALE_FACTOR * zoom;
		...
		this.buffer = createCompatibleImage(imgWidth, imgHeight); //new BufferedImage((int)this.width, (int)this.height, BufferedImage.TYPE_INT_ARGB);
		this.backBuffer = createCompatibleImage(imgWidth, imgHeight); //new BufferedImage((int)this.width, (int)this.height, BufferedImage.TYPE_INT_ARGB);
		revalidate();
		paintBuffered();
	}
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.anim.model.AnimationListener#updateModelTime(long)
	 */
	...
	protected void paintBuffered() {
		...
		g2d.fill(new Rectangle2D.Float(0, 0, backBuffer.getWidth() + 1, backBuffer.getHeight() + 1));
		// translate origin
