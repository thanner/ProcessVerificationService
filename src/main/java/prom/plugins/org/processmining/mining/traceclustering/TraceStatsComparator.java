...
public abstract class TraceStatsComparator {
	...
	public TraceStatsComparator(TraceStats traceStats) {
		...
		for(int x=0; x<size; x++) {
			for(int y=0; y<size; y++) {
				//relations.set(x, y, (relations.get(x, y) * normalizationFactor));
	...
	protected class TraceComparator implements Comparator<SingleTraceStat> {
		...
		public int compare(SingleTraceStat one, SingleTraceStat two) {
			double toOne = comparator.getRelation(reference.getTraceIndex(), one.getTraceIndex());
			double toTwo = comparator.getRelation(reference.getTraceIndex(), two.getTraceIndex());
			if(toOne == toTwo) {
				return 0;
			} else if(toOne > toTwo) {
				return 1;
