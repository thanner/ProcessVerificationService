...
public class AttenuationDisplayPanel extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		g2d.drawLine(insets.left + 10, insets.top + 10, insets.left + 10, insets.top + height - 40);	// y-axis
		g2d.drawLine(insets.left + 10, insets.top + height - 40, width - 10, insets.top + height - 40); // x-axis
		...
		for(int i=1; i<=maxDistance; i++) {
			...
			barHeight = (int)((double)maxBarHeight * attenuation.getAttenuationFactor(i));
			barY = insets.top + 10 + maxBarHeight - barHeight;
			g2d.fillRect(barX, barY, barWidth, barHeight);
			g2d.setColor(textColor);
			g2d.drawString(Integer.toString(i), barX + 3, insets.top + height - 15);
			barX += barWidth;
			barX += barWidth;
		}
		g2d.dispose();
