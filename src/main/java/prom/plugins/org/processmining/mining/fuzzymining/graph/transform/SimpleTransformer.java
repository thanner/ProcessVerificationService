...
public class SimpleTransformer extends FuzzyGraphTransformer {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.graph.transform.FuzzyGraphTransformer#transform(org.processmining.mining.fuzzymining.graph.FuzzyGraph)
	 */
	...
	protected boolean shouldMerge(ClusterNode first, ClusterNode second) {
		if(first.equals(second)) {
			...
			preBoth.addAll(preFirst);
			preBoth.retainAll(preSecond);
			...
			*/
			...
			postBoth.addAll(postFirst);
			postBoth.retainAll(postSecond);
			if((postBoth.size() * 2) > (postFirst.size() + postSecond.size())) {
				return true;
			} else {
				return false;
			}
