	...
	protected double measure(AuditTrailEntry reference, AuditTrailEntry follower) {
		...
		for(String key : refKeySet) {
			if(folKeySet.contains(key)) {
				keyOverlap++;
				/*
				if(refMap.get(key).equals(folMap.get(key))) {
					valOverlap++;
				}
				*/
				double editDistance = StringSimilarity.similarity(refMap.get(key), folMap.get(key));
				double maxLength = Math.max(refMap.get(key).length(), folMap.get(key).length());
