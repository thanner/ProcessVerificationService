...
public class ClusterNode extends Node implements DotFileWriter {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.graph.Node#directlyFollows(org.processmining.mining.fuzzymining.graph.Node)
	 */
	...
	public void writeToDot(Writer bw) throws IOException {
		bw.write(id() + " [shape=\"octagon\",style=\"filled\",fillcolor=\"chartreuse3\",label=\"Cluster " 
	...
	public JPanel getClusterGraphPanel() throws Exception {
		GGNodePainter externalNodePainter = new GGNodeRectanglePainter(new Color(220, 220, 220), new Color(50, 50, 50), new Color(90, 90, 90));
		GGNodePainter externalClusterPainter = new GGNodeOctagonPainter(new Color(180, 220, 180), new Color(50, 50, 50), new Color(70, 70, 70));
		GGNodePainter internalNodePainter = new GGNodeRoundRectanglePainter(new Color(160, 140, 100), new Color(20, 20, 20), new Color(40, 40, 40));
		...
		for(Node node : adjacentNodes) {
			...
			String[] label = new String[] {node.getElementName(), node.getEventType(), MutableFuzzyGraph.format(getSignificance())};
		...
		for(Edge edge : externalEdges.getEdges()) {
			GGEdge ezEdge = new GGEdge(ezNodeMap.get(edge.getSource().id()), 
					ezNodeMap.get(edge.getTarget().id()), null);
			ezEdge.setPainter(new GGEdgeGenericPainter(new Color(150, 150, 150), new Color(120, 120, 120)));
			clusterGraph.add(ezEdge);
			ezEdge.setAttribute(GGAdapter.ATTR_FUZZY_GRAPH, graph);
			ezEdge.setAttribute(GGAdapter.ATTR_FUZZY_GRAPH_ELEMENT, edge);
		}
	...
	public void writeClusterSubgraphToDot(Writer bw) throws IOException {
		// write DOT prelude
		bw.write("digraph CLUSTERGRAPH_" + index +" { ranksep=\".3\"; fontsize=\"8\"; remincross=true; margin=\"0.0,0.0\"; ");
	...
	protected void addEdgesBetweenSets(Set<Node> sources, Set<Node> targets, Edges edges) {
		int x, y;
		double sig, cor;
