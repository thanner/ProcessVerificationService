...
public class TraceScatterMetric extends TraceMetric {
	 ...
	 * @param log
	 */
	public TraceScatterMetric(LogSummary logSummary) {
		super("Trace scatter",
				"Measures traces by their number of distinct event classes relative to their overall size.",
				logSummary);
	}

	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.metrics.TraceMetric#measure(org.processmining.framework.log.LogReader)
	 */
	public void measure(LogReader log) {
		...
		for(int i=0; i<log.numberOfInstances(); i++) {
			...
			values[i] = ((double)eventSet.size() / (double)ateList.size());
		}
	}
