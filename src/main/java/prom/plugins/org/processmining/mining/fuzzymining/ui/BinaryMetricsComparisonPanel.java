...
public class BinaryMetricsComparisonPanel extends JPanel {
	...
	public BinaryMetricsComparisonPanel(MetricsRepository metricsRepository) {
		...
		events = metricsRepository.getLogReader().getLogSummary().getLogEvents();
		...
		metrics.add(metricsRepository.getAggregateSignificanceBinaryLogMetric());
		...
		binMetricsControlPanel.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
		binMetricsControlPanel.setLayout(new BoxLayout(binMetricsControlPanel, BoxLayout.X_AXIS));
	...
	protected void displayMetric(BinaryMetric metric) {
		...
		graphPanel.add(new BinaryMetricsPanel(metric, events), BorderLayout.CENTER);
		this.validate();
		repaint();
	}
}
