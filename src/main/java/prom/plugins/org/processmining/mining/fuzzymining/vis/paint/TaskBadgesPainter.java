...
public class TaskBadgesPainter extends Painter {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.vis.paint.Painter#paint(long, java.awt.Graphics2D)
	 */
	@Override
	public void paint(long modelTime, Graphics2D g2d) {
		...
		taskAnimations.addAll(canvas.getAnimation().getPrimitiveTaskAnimations());
		...
		for(TaskAnimation taskAnimation : taskAnimations) {
			...
			// derive colors
			emergedBgColor = ColorSettings.fade(canvas.getColorSettings().getTaskBadgeBackgroundEmerged(), percentage);
			swallowedBgColor = ColorSettings.fade(canvas.getColorSettings().getTaskBadgeBackgroundSwallowed(), percentage);
			traversedBgColor = ColorSettings.fade(canvas.getColorSettings().getTaskBadgeBackgroundTraversed(), percentage);
			textColor = ColorSettings.fade(canvas.getColorSettings().getTaskBadgeForeground(), percentage);
			// derive node coords
			...
			if(emergeCount > 0) {
				...
				Rectangle2D emergedBounds = fMetrics.getStringBounds(emerged, g2d);
				...
				RoundRectangle2D.Float rrect = new RoundRectangle2D.Float(
						badgeX, 
						badgeY, 
						(int)emergedBounds.getWidth() + badgeHBorder + badgeHBorder, 
						badgeHeight, 
				...
				triangle.lineTo(badgeX - badgeHBorder, badgeY + (badgeHeight / 2));
				triangle.lineTo(badgeX + 0.5f, badgeY + badgeHeight - badgeHBorder);
				triangle.closePath();
				g2d.fill(triangle);
				// paint string
				float sX = badgeX + badgeHBorder;
				float sY = badgeY + (badgeHeight / 2) + (float)(2 * (emergedBounds.getHeight() / 5));
			...
			if(swallowCount > 0) {
				...
				Rectangle2D swallowedBounds = fMetrics.getStringBounds(swallowed, g2d);
				...
				RoundRectangle2D.Float rrect = new RoundRectangle2D.Float(
						badgeX, 
						badgeY, 
						(int)swallowedBounds.getWidth() + badgeHBorder + badgeHBorder, 
						badgeHeight, 
						badgeRadius, badgeRadius);
				g2d.fill(rrect);
				// triangle pointer
				...
				// paint string
				float sX = badgeX + badgeHBorder;
				float sY = badgeY + (badgeHeight / 2) + (float)(2 * (swallowedBounds.getHeight() / 5));
				g2d.setColor(textColor);
			...
			if(traverseCount > 0) {
				...
				Rectangle2D traversedBounds = fMetrics.getStringBounds(traversed, g2d);
				g2d.setColor(traversedBgColor);
				float badgeWidth = (float)traversedBounds.getWidth() + badgeHBorder + badgeHBorder;
				...
				triangle.lineTo(badgeX + badgeWidth + badgeHBorder, badgeY + (badgeHeight / 2));
				triangle.lineTo(badgeX + badgeWidth , badgeY + badgeHeight - badgeHBorder);
				triangle.closePath();
				g2d.fill(triangle);
				// paint string
				float sX = badgeX + badgeHBorder;
				float sY = badgeY + (badgeHeight / 2) + (int)(2 * (traversedBounds.getHeight() / 5));
				g2d.setColor(textColor);
