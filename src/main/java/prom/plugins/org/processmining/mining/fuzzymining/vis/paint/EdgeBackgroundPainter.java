...
public class EdgeBackgroundPainter extends Painter {
	...
	public void paint(long modelTime, Graphics2D g2d) {
		...
		long idleTime;
		int traverseCount;
		int bestQuarterTraverseCount = canvas.getAnimation().getBestQuarterTraverseCount();
		float percentage;
		...
		for(ArcAnimation anim : arcAnimations) {
			...
			percentage = (float)traverseCount / (float)bestQuarterTraverseCount;
			if(percentage > 1.0f) {
				percentage = 1.0f;
			}
			g2d.setStroke(new BasicStroke(canvas.getLineWidth(percentage), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
			// derive stroke color from idel time and set
			idleTime = modelTime - keyframe.getLastCompleted();
			Color terminalColor;
			if(idleTime > fadeTime) {
				terminalColor = canvas.getColorSettings().getPassiveEdgeBackground();
			} else {
				percentage = (float)idleTime / (float)fadeTime;
				terminalColor = ColorSettings.mix(
						canvas.getColorSettings().getActiveEdgeBackground(), 
						canvas.getColorSettings().getPassiveEdgeBackground(), 
						percentage);
			}
			...
			/*
			GradientPaint gradient = new GradientPaint(firstX, firstY, sourceColor, 
					currentX, currentY, terminalColor, false);
