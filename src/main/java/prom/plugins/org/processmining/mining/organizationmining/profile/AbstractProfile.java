...
public abstract class AbstractProfile extends Profile {
	...
	protected AbstractProfile(String aName, String aDescription, LogReader aLog) {
		...
		values = new EfficientSparseDoubleArray[log.getLogSummary().getOriginators().length];
	...
	public double getValue(String originatorName, int itemIndex) {
		return normalize(values[originators.indexOf(originatorName)].get(itemIndex));
	}
	 ...
	 * @param increment
	 */
	protected void incrementValue(String originatorName, String itemId, double increment) {
		...
		if(itemIndexObj != null) {
			itemIndex = itemIndexObj;
			currentValue = values[originators.indexOf(originatorName)].get(itemIndex);
		} else {
			// not present yet, add
			itemIndex = itemKeys.size();
			itemKeys.add(itemId);
		...
		values[originators.indexOf(originatorName)].set(itemIndex, currentValue);
	}
