...
public class FuzzyGraphEditorPanel extends JPanel {
	...
	public void showNodeConfiguration(GGNode node) {
		...
		sidePanel = new SidePanel(new FuzzyNodeEditor(this, node, closeListener));
	...
	public void showEdgeConfiguration(GGEdge edge) {
		...
		sidePanel = new SidePanel(new FuzzyEdgeEditor(this, edge, closeListener));
		innerPanel.add(sidePanel, BorderLayout.WEST);
		this.revalidate();
		this.repaint();
	}
	...
	protected JPanel getOverviewPanel() {
		...
		nNodeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				graph.getNodeSignificanceMetric().setMeasure(node.getIndex(), 1.0);
				Thread updateThread = new Thread() {
					public void run() {
						final GGGraph gGraph = updateGraphLayout();
						for(GGNode gNode : gGraph.nodes()) {
							Node n = (Node)gNode.getAttribute(GGAdapter.ATTR_FUZZY_GRAPH_ELEMENT);
							if(n.equals(node)) {
		...
		nEdgeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				final int source = ((Node)sourceBox.getSelectedItem()).getIndex();
				final int target = ((Node)targetBox.getSelectedItem()).getIndex();
				graph.setBinarySignificance(source, target, 1.0);
				graph.setBinaryCorrelation(source, target, 1.0);
				graph.getEdgeCorrelationMetric().setMeasure(source, target, 1.0);
				graph.getEdgeSignificanceMetric().setMeasure(source, target, 1.0);
				Thread updateThread = new Thread() {
					public void run() {
						final GGGraph gGraph = updateGraphLayout();
						for(GGEdge gEdge : gGraph.edges()) {
							Edge e = (Edge)gEdge.getAttribute(GGAdapter.ATTR_FUZZY_GRAPH_ELEMENT);
							if(e.getSource().getIndex() == source && e.getTarget().getIndex() == target) {
