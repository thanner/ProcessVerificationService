...
public class AnimationTriggerPanel extends JPanel {
	...
	public AnimationTriggerPanel(FuzzyMiningResult result) {
		...
		try {
			discretize = (result.getLogReader().getInstance(0).getAuditTrailEntryList().get(0).getTimestamp() == null);
		} catch(Exception e) {
			e.printStackTrace();
		}
		...
		animButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				LogFilter projectionFilter = new FuzzyGraphProjectionFilter(refResult.getGraph());
				projectionFilter.setLowLevelFilter(refResult.getLogReader().getLogFilter());
				...
				try {
					LogReader filteredLog = LogReaderFactory.createInstance(projectionFilter, refResult.getLogReader());
					AnimationFrame animFrame = new AnimationFrame(filteredLog, refResult.getGraph(), 
							lookaheadSlider.getValue(), extraLookaheadSlider.getValue());
					MainUI.getInstance().createFrame("Fuzzy graph animation", animFrame);
				} catch (Exception e1) {
