...
public class RoutingSignificanceMetric extends UnaryDerivateMetric {
	...
	/* (non-Javadoc)
	 * @see org.processmining.mining.fuzzymining.metrics.UnaryDerivateMetric#measure()
	 */
	public void measure() {
		int size = repository.getLogReader().getLogSummary().getLogEvents().size();
		...
		for(int i=0; i<size; i++) {
			...
			for(int x=0; x<size; x++) {
				if(x == i) { continue; }	// skip self-references
