 ...
 *
 */
public class BinaryMetricsPanel extends JComponent implements MouseListener, MouseMotionListener {
	...
	protected void paintComponent(Graphics g) {
		...
		if(oldBlockSize != blockSize || matrixBuffer == null) {
			...
			matrixBuffer = new BufferedImage(size * blockSize, size * blockSize, BufferedImage.TYPE_INT_RGB);
			...
			for(int a = 0; a < size; a++) {
				...
				for(int b = 0; b < size; b++) {
					gBuf.setColor(ColorRepository.getGradualColorBlackZero(metric.getMeasure(a, b)));
					if(blockSize > 1) {
		...
		if(mouseX >= 0 && mouseX < (size * blockSize) && mouseY >= 0 && mouseY < (size * blockSize)) {
			// draw info
	...
	protected void paintInfo(Graphics2D g2d, int eventX, int eventY, int x, int y) {
		g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		double value = metric.getMeasure(eventX, eventY);
		String evtX = events.get(eventX).getModelElementName() + " (" + events.get(eventX).getEventType() + ")";
		String evtY = events.get(eventY).getModelElementName() + " (" + events.get(eventY).getEventType() + ")";
