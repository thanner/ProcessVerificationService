...
public abstract class AbstractProfile extends Profile {
	...
	/**
	 * Array, which holds for each process instance a
	 * sparse array, holding the values (Double precision) for each item.
	 */
	protected EfficientSparseDoubleArray[] values;// SparseArray<Double>[] values;
	 ...
	 * @param aDescription human-readable, 1-sentence description of the profile
	 * @param log LogReader instance used to build the profile
	 */
	protected AbstractProfile(String aName, String aDescription, LogReader aLog) {
		...
		values = new EfficientSparseDoubleArray[log.getLogSummary().getNumberOfProcessInstances()];
	...
	/* (non-Javadoc)
	 * @see org.processmining.analysis.traceclustering.profile.Profile#getItemKey(int)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see org.processmining.analysis.traceclustering.profile.Profile#numberOfItems()
	 */
	@Override
	...
	public double getValue(int instanceNumber, int itemIndex) {
		return normalize(values[instanceNumber].get(itemIndex));
	}

	public double getValue(int instanceNumber, String itemId) {
	...
	/**
	 * Manually sets the value for a specific process instance /
	 * item combination in this profile
	 * @param instanceId
	 * @param itemId
	 * @param value
	 */
	...
	/**
	 * Increments the value stored for a specific process instance /
	 * item combination stored in this profile, by the given increment
	 * @param instanceId
	 * @param itemId
	 * @param increment
	 */
	protected void incrementValue(int instanceIndex, String itemId, double increment) {
		...
		if(itemIndexObj != null) {
			itemIndex = itemIndexObj;
			currentValue = values[instanceIndex].get(itemIndex);
		} else {
			// not present yet, add
			itemIndex = itemKeys.size();
			itemKeys.add(itemId);
		...
		values[instanceIndex].set(itemIndex, currentValue);
	}

	public DistanceMatrix getDistanceMatrix(DistanceMetric metric, Progress progress) {
