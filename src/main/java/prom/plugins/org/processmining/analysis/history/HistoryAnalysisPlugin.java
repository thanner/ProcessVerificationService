...
/**
 * Enhances the input log with history information, i.e., which events were executed how 
 * many times in the current state of the process. 
 * This information is recorded in dedicated data attributes for each audit trail entry. 
 * <br>
...
public class HistoryAnalysisPlugin implements AnalysisPlugin {
	...
	public String getHtmlDescription() {
		return ("<h1>" + getName() + "</h1>" + 
				...
				"So, the history is captured without including the current log event itself. <BR>" +
				"The purpose of this plugin is to enable the analysis of the history via data mining techniques, e.g., in the Decision Miner.");
	}
	...
	public JComponent analyse(AnalysisInputItem[] analysisInputItemArray) {
		...
		//Iterator<ProcessInstance> allInstances = historyLog.instanceIterator();
		...
		while (allInstances.hasNext()) {
			...
			while (allAteIt.hasNext()) {
				...
				countingMeasures.put(currentATE.getModelElementName()+currentATE.getEventType(), 0);
			}
			...
			try {
				...
				while (index < entries.size()) {
					...
					Iterator<String> soFarCounted = countingMeasures.keySet().iterator();
					...
					if (countingMeasures.containsKey(name+type) == true) { 
						// just increment if this ate was already counted at least once
						int count = countingMeasures.get(name+type);
						count = count + 1;
						countingMeasures.put(name+type, count);
					}
					else {
						// should never happen as counting measures are initialized
					}
				...
				Message.add("An error occurred during filtering process instance " + currentPI.getName());
				ex.printStackTrace();
			}
		}
		
		//return new HistoryAnalyisResult(this, analysisInputItemArray, historyLog);
