...
/**
 * Base metric for all MDL metrics that measure how much the model allows
 * to "compress" the log. The idea is that it should be more efficient to specify
 * the log with respect to a model than to explicitly give the log (as, e.g., in a sequence
 * it is clear which is the next element depending on the current log replay status -
 * so no encoding effort is needed at all).
 * <br>
 * The so-called "preciseness" metrics are based on the log replay as implemented
 * in the ProM framework, and may or may not deal with errors (i.e., fitness problems).
 *
...
public abstract class MDLPrecisenessMetric extends MDLBaseMetric {
	 ...
	 * @param aName the name of this metric (to be passed to super)
	 * @param aDescription the description of this metric (to be passed to super)
	 */
	...
	public int getEncodingCost(ProgressPanel progress, PetriNet aNet, LogReader aLog) {
		...
		LogReplayAnalysisMethod logReplayAnalysis = new LogReplayAnalysisMethod(net, log, measurer, progress);
	...
	/**
	 * Custom measurer that collects data during log replay that is needed to calculate
	 * the enconding costs of the corresponding "preciseness" metric.
	 *
	 * @author Anne Rozinat
	 */
	abstract class PrecisenessMeasurer extends Measurer {
		...
		/**
		 * Measurer collected data during log replay.
		 * This is now used to calculate the total encoding costs.
		 *
		...
		protected void initTraceReplay(ReplayedLogTrace pi, LogReplayAnalysisResult result) {
			traceEncodingCost = 0;
			noOfErrors = 0;
			groupedInstances = MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(pi.getProcessInstance());
		}

		/**
		 * Measures the Number of enabled tasks at current replay stage (before replay of the ate).
		 */
		protected void takePreStepExecutionMeasurement(LogReplayAnalysisResult result, ReplayedLogTrace pi, int maxDepth) {
			int noOfEnabledTasks = determineNumberOfEnabledTasks(result, pi, maxDepth);
		...
		protected void takePostTraceReplayMeasurement(LogReplayAnalysisResult result,
				ReplayedLogTrace pi) {
			totalEncodingCost = totalEncodingCost + (groupedInstances * getUpperBound(traceEncodingCost));
		}

		////////////////////////////// Custom methods ////////////////////////////

		/**
		 * Use the number of enabled tasks (measured before each replay step)
		 * to calculate different encoding costs.
		 *
		...
		/**
		 * TODO: move this method to log replay package to avoid code duplication
		 *
		 * Determines the number of enabled tasks while transparently treating invisible tasks.
		 * @param result
		 * @param pi
		 * @param maxDepth
		 * @return
		 */
		protected int determineNumberOfEnabledTasks(LogReplayAnalysisResult result, ReplayedLogTrace pi, int maxDepth) {
			...
			// (add 1 as we want to look at enabled tasks from current replay marking)
			...
			alreadyCounted = traceEnabledTasks(alreadyCounted, coverabilityGraph, replayState, maxEnabled, visitedStates);
