...
public class EditHighLevelProcessGui extends JPanel implements Provider, MiningResult {
	...
	/**
	 * Creates a high-level process GUI based on a HLModel plus its
	 * HLProcess.
	 * <p>A generic visualization will be provided to highlight
	 * nodes in the process model that are associated to the high level information.
	 * @param process the HLModel of whicht the high-level information should be made editable
	 */
	...
	/**
	 * Creates a high-level process GUI based on an HLProcess only.
	 * <br>This means that there is no HLModel available that links
	 * high-level process information to certain nodes in the process model.
	 * @param hlprocess the HLProcess to be made editable
	 */
	...
	public ProvidedObject[] getProvidedObjects() {
		if (hlProcessImpl != null) {
			return new ProvidedObject[] {
					new ProvidedObject("High-level Process", new Object[] {hlProcessImpl}),
					new ProvidedObject("Process Information", new Object[] {hlProcess})
			};
		}
		else {
			return new ProvidedObject[] {
