...
public class StreamView extends JComponent {
	...
	protected AffineTransform shadowTransform = AffineTransform.getTranslateInstance(2, 3);
	
	protected SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yy HH:mm:ss.SSS");
	...
	public void setActiveIndex(int index) {
		...
		if(index >= 0) {
			...
			this.scrollRectToVisible(new Rectangle(x - 100, 0, 200, getHeight()));
		}
		repaint();
	}
	...
	protected void paintComponent(Graphics g) {
		...
		g2d.draw(new RoundRectangle2D.Float(0, 0, this.getWidth() - 1, this.getHeight() - 1, 10, 10));
		...
		for(int index=startX; (index < endX && index < ateList.size()); index++) {
			...
			if(currentNode != null) {
				...
				float yTop = northBorder + (currentNode.getMinIndex() * vRaster);
				float yBottom = northBorder + (currentNode.getMaxIndex() * vRaster);
				g2d.fill(new Rectangle2D.Float(x, yTop, hRaster, yBottom - yTop));
			}
		}
		// paint range
		for(int index=startX; (index < endX && index < ateList.size()); index++) {
		...
		Rectangle2D traceNameBounds = g2d.getFontMetrics().getStringBounds(name, g2d);
		if(clip.height > 25 && clip.width > traceNameBounds.getWidth() + 10) {
			GeneralPath flag = new GeneralPath();
			flag.moveTo(clip.x + 1, clip.y + 5);
			flag.lineTo(clip.x + (float)traceNameBounds.getWidth() + 10, clip.y + 5);
			flag.lineTo(clip.x + (float)traceNameBounds.getWidth() + 17, clip.y + 15);
			flag.lineTo(clip.x + (float)traceNameBounds.getWidth() + 10, clip.y + 25);
		...
		if(mouseX > 0 && mouseY > 0) {
			...
			if(eventIndex >= 0 && eventIndex < ateList.size()) {
				...
				float inspectorY = northBorder + (ecIndex * vRaster) + (vRaster / 2);
				Rectangle2D nameBounds = g.getFontMetrics().getStringBounds(eventName, g2d);
				Rectangle2D dateBounds = g.getFontMetrics().getStringBounds(date, g2d);
				float inspectorWidth = (float)Math.max(nameBounds.getWidth(), dateBounds.getWidth());
				...
				g2d.setColor(colorShadow);
				g2d.fill(shadowPath);
				RoundRectangle2D.Float bg = new RoundRectangle2D.Float(mouseX + 20, inspectorY - 15, inspectorWidth + 10f, 30, 10, 10);
				...
				g2d.drawString(date, mouseX + 26, inspectorY + (float)dateBounds.getHeight() + 2);
				g2d.setColor(colorInspectorText);
				g2d.drawString(eventName, mouseX + 25, inspectorY - 2);
				g2d.drawString(date, mouseX + 25, inspectorY + (float)dateBounds.getHeight());
			}
		}
	}
	...
	protected void paintIndex(int index, Graphics2D g2d) {
		if(buffer == null) {
			...
			if(vSize > 1 && hSize > 1) {
				buffer = new BufferedImage((int)Math.ceil(hSize), (int)Math.ceil(vSize), BufferedImage.TYPE_INT_ARGB);
				Graphics2D g2db = buffer.createGraphics();
				g2db.setColor(colorBlip);
				g2db.fill(new Rectangle2D.Float(fuzziness * hRaster, 0, hRaster, vRaster));
				GradientPaint gradient = new GradientPaint(0, 0, colorTransparent, fuzziness * hRaster, 0, colorBlip, false);
				g2db.setPaint(gradient);
				g2db.fill(new Rectangle2D.Float(0, 0, fuzziness * hRaster, vRaster));
				gradient = new GradientPaint((fuzziness * hRaster) + hRaster, 0, colorBlip, (2 * fuzziness * hRaster) + hRaster, 0, colorTransparent, false);
				g2db.setPaint(gradient);
				g2db.fill(new Rectangle2D.Float((fuzziness * hRaster) + hRaster, 0, (fuzziness * hRaster), vRaster));
				g2db.dispose();
			}
		}
		try {
			...
			if(buffer == null) {
				...
				g2d.drawImage(buffer, (int)(x - (hRaster * fuzziness)), (int)y, null);
			}
			// draw active indicator, if appropriate
			if(index == activeIndex) {
				float radius = hRaster * 1.2f;
				Ellipse2D indicator = new Ellipse2D.Float(x - radius, y - radius, radius * 2, radius * 2);
				g2d = (Graphics2D)g2d.create();
				g2d.setColor(new Color(220, 220, 220, 80));
				g2d.fill(indicator);
				g2d.setColor(new Color(230, 230, 230, 220));
				g2d.setStroke(new BasicStroke(2f));
				g2d.draw(new Line2D.Float(x + radius, y, x + radius, y - 4 * radius));
				g2d.draw(indicator);
				g2d.dispose();
			}
			/*
			 * NOTE: location bar painting temporarily removed (find less distracting alternative)
			int eventIndex = (int)Math.floor((mouseX - westBorder) / hRaster);
			if(eventIndex == index) {
				// draw focus indicator
				//g2d.setColor(new Color(20, 160, 20, 40));
				//g2d.draw(new Rectangle2D.Float(x - 3, 3, hRaster + 6, getHeight() - 6));
				g2d.setColor(new Color(20, 160, 20, 30));
