...
/**
 * Class holding a decision point and a 
 * log, and providing a method to split up the log 
 * in instances corresponding to loop occurrences
 * (ignoring previous instance divisions).
 */
public class DecisionPointPlusLog {
	 ...
	 * @param newDecisionPoint the new decision point
	 */
	...
	/**
	 * Creates a new log where each process instances is split up 
	 * into multiple instances if there are loops. 
	 * <br>
	 * For each observed target category concept of the decision point 
	 * a new instances is created (up to this point).
	 * This is useful in conjunction with the decision miner, which only 
	 * interprets the last occurrence of a decision within an instance
	 * and thus ignores all previous occurrences of the same loop.
	 * @return the log reader consisting of separate instances for 
	 * each loop occurrence
	 */
	public LogReader getLoopSeparatedLog() {
		...
		try {
			File outputFile = File.createTempFile("DecisionPointPlusLogTemp", ".mxml.gz");
			...
			for (int i=0; i<log.numberOfProcesses(); i++) {
				...
				for(int j=0; j<process.size(); j++) {
					...
					persistency.startProcessInstance(name + "_0", description, instance.getAttributes());
					int lastSplit = -1;
					for (int k=0; k<ateList.size(); k++) {
						...
						LogEvent lePendent = new LogEvent(ate.getElement(), ate.getType());
						if (separatingEvents.contains(lePendent) == true && lastSplit < k) {
							persistency.endProcessInstance();
							persistency.startProcessInstance(splitname, description, instance.getAttributes());
							lastSplit = k;
							k = 0;
						}
					}
			...
			LogReader loopSeparated = BufferedLogReader.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), logFile);
			outputFile.deleteOnExit();
			return loopSeparated;	
		} catch (Exception ex) {
			ex.printStackTrace();
			Message.add("Error occured while creating loop-separated log. Non-separated log exported instead.");
			return log;
		}
	}
