...
public class StrAppropriatenessVisualization extends DiagnosticPetriNet {
	...
	protected void writeTransitionsToDot(Writer bw) throws IOException {
		...
		if (redundantInvisiblesOption == false && alternativeDuplicatesOption == false) {
			// use default
			...
			while (it.hasNext()) {
				...
				try {
					...
					if (redundantInvisiblesOption == true &&
							myDiagnosticTransition.isRedundantInvisibleTask()) {
					...
					// mark with different fill color
							...
							myDiagnosticTransition.isAlternativeDuplicateTask()) {
						bw.write("t" + myDiagnosticTransition.getNumber() +
