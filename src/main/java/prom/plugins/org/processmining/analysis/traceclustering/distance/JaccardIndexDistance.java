		...
		for(int x=0; x<numberOfInstances; x++) {
			for(int y=x+1; y<numberOfInstances; y++) {
				...
				for(int i=0; i<profile.numberOfItems(); i++) {
					p += profile.getValue(x, i)*profile.getValue(y, i);
					q += profile.getValue(x, i)*profile.getValue(x, i);
					r += profile.getValue(y, i)*profile.getValue(y, i);
/*					
...
/*	@Override
	public DistanceMatrix getDistanceMatrix(Set<Profile> profiles) {
		int numberOfInstances = profiles.iterator().next().numberOfInstances();
		DistanceMatrix distanceMatrix;
		if(numberOfInstances < 2000) {
			distanceMatrix = new DoubleDistanceMatrix(numberOfInstances);
		} else {
