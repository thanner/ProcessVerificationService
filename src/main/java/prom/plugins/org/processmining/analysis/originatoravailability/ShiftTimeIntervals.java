...
/**
 * Calculates the shift of the day for which a given time belongs. Every shift has 
 * a starting date and an ending date.
 * 
 * For instance, if the shift is equal to six hours, than the times of the
 * days will fit in one of the following four time shift intervals: 
 * (i) 0:00h-5:59h, (ii) 6:00h-11:59h, (iii) 12:00h-17:59h, and (iv) 18:00h-23:59h.
 *  
...
public class ShiftTimeIntervals {
	...
	/**
	 * Calculates the time intervals based on the value for the
	 * constant ShiftTimeIntervals.DEFAULT_HOURS_PER_SHIFT
	 */
	 ...
	 * hoursPerShift.
	 * @param Number of ours per shift of a resource.
	 */
	...
	/**
	 * Identifies the shift for which a given time belongs. Every shift has 
	 * a starting date and an ending date.  
	 * @param timestamp Given time to calculate the shift for.
	...
	public ComparablePair<Date, Date> getShift(Date timestamp) {
		...
		cal.set(Calendar.HOUR_OF_DAY, (int) Math.floor((cal.get(Calendar.HOUR_OF_DAY) % 24) / hoursPerShift) * hoursPerShift);
