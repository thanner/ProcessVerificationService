...
public class LogMetrics {
	...
	public String formatMetrics() {
		...
		sb.append("-- min: " + minTraceTimeGranularity + " (log time granularity)\n");
	...
	protected void scanMetrics(LogReader log) throws IOException {
		...
		for(int i=0; i<log.numberOfInstances(); i++) {
			...
			for(int e=0; e<ateList.size(); e++) {
				magnitude++;
				ate = ateList.get(e);
				traceEventClasses.add(logEvents.findLogEvent(ate.getElement(), ate.getType()));
			...
			traceStructure[i] = 1.0 - (traceBinRelations[i].size() / 
					(double)(traceEventClasses.size() * traceEventClasses.size()));
		...
		logStructure = 1.0 - (logBinRelations.size() / (double)(logEvents.size() * logEvents.size()));
		// calculate affinities
		logAffinity = 0.0;
		traceAffinity = new double[log.numberOfInstances()][log.numberOfInstances()];
		...
		for(int x=0; x<log.numberOfInstances(); x++) {
			for(int y=x+1; y<log.numberOfInstances(); y++) {
				...
				logAffinity += affinity; // 2 x, because we save one comparison each!
		...
		logAffinity /= (double)(log.numberOfInstances() * (log.numberOfInstances() - 1));
	}
	...
	protected double calculateAffinity(int traceIndexA, int traceIndexB) {
		double union = traceBinRelations[traceIndexA].union(traceBinRelations[traceIndexB]);
		double overlap = traceBinRelations[traceIndexA].overlap(traceBinRelations[traceIndexB]);
		if(union == 0) {
			return 0;
		} else {
