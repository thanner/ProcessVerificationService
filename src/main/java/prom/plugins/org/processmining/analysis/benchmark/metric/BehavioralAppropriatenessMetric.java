...
/**
 * The behavioral appropriateness metric builds on global successorship and predecessorship relations. 
 * It looks for "sometimes" relations (i.e., alternative or parallel behavior) in the model and the log, 
 * and punishes those relations that are in the model but not in the log.
 * <br>
 * Values range from 0 to 1. Note that metric is relative to model flexibility (the less flexible the model
 * is, the more coase-grained the measurement steps). In the case of a purely sequential model, this 
 * metric is trivially true. Furthermore, the flower model can yield a value of 1.0 if the log itself also exhibits 
 * random behavior. 
 * <br>
 * Assumptions: Can deal with duplicate tasks. Big loops (i.e., spanning huge parts of the model) will "blur" the
 * measurements (behavior with loops cannot be analyzed properly).
 * 
 * @author Anne Rozinat
 */
public class BehavioralAppropriatenessMetric implements BenchmarkMetric {
	...
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel, Progress progress) {
		...
		StateSpaceExplorationMethod stateSpaceAnalysis = new StateSpaceExplorationMethod(model, progress);
		StateSpaceExplorationResult stateSpaceResult = (StateSpaceExplorationResult) stateSpaceAnalysis.analyse(myOptions);
		
		// clean up previous results first
		// TODO: check whether needed (might be obsolete as result object only used once)
		...
		// return result
		double behavioralAppropriateness = stateSpaceResult.getImprovedBehavioralAppropriatenessMeasure();
	...
	/**
	 * Creates the configuration object that is used by the log replay and state space exploration to determine 
