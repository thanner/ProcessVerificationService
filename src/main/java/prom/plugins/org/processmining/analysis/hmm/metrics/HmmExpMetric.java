...
/**
 * Base class for metrics to be used in HMM experiment. 
 * Handles metric-specific gnuplot script writing. 
 * 
 * @author Anne Rozinat (a.rozinat at tue.nl)
 */
public abstract class HmmExpMetric {
	
	public enum MetricType { Fitness, Precision }
	public enum ExpType { Noise, Completeness }
	...
	public HmmExpMetric(String aFolder, String aName, MetricType metType, ExpType expType) {
		folder = aFolder;
		name = aName;
		mType = metType;
		eType = expType;
		...
		valueRatioWriter = HmmExpUtils.createWriter(folder, filename + "Ratio", "dat");
		// for replications
		averageWriter = HmmExpUtils.createWriter(folder, filename + "Average", "dat");
		averageRatioWriter = HmmExpUtils.createWriter(folder, filename + "AverageRatio", "dat");
		varianceWriter = HmmExpUtils.createWriter(folder, filename + "Variance", "dat");
	}
	...
	public void writeRatioGnuplot() {
		GnuplotScript gnuplot = new GnuplotScript(folder, filename + "Ratio", 
				name + " " + mType + " Ratio for Different " + eType + " Levels",
				eType + " Level", mType + " Ratio", PlotType.lines);
	...
	public void writeAverageRatioGnuplot() {
		GnuplotScript gnuplot = new GnuplotScript(folder, filename + "Ratio", 
				name + " " + mType + " Ratio and Variance for Different " + eType + " Levels",
				eType + " Level", mType + " Ratio", PlotType.lines);
	...
	public abstract double calculateValue(HmmAnalyzer analyzer, PetriNet pnet, LogReader log);
