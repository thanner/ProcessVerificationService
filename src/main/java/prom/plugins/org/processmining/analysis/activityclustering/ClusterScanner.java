...
public class ClusterScanner {
	...
	public ClusterScanner(LogReader log, CorrelationMetric metric, int lookahead, double threshold) {
		this.log = log;
		this.metric = metric;
	...
	public List<Cluster> scanInstance(int index) 
		...
		for(int i=0; i<ateList.size(); i++) {
			scanForwardChained(i, index, bestLookaheadFellows, clusterMap, clusters);
		}
		return clusters;
	}
	
	protected void scanForwardChained(int init, int index, int[] bestLookaheadFellows, 
			Cluster[] clusterMap, List<Cluster> clusters) throws IndexOutOfBoundsException, IOException {
	...
	protected int findBestLookaheadFellow(AuditTrailEntryList ateList, int eventIndex) 
			throws IndexOutOfBoundsException, IOException {
