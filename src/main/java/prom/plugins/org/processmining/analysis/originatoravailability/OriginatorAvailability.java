...
public class OriginatorAvailability {
	...
	public OriginatorAvailability(LogReader log, ShiftTimeIntervals intervals) {
		...
		for (ProcessInstance pi : log.getInstances()) {
			for (AuditTrailEntry ate : pi.getListOfATEs()) {
				ComparablePair<Date, Date> shift = intervals.getShift(ate.getTimestamp());
				shifts.add(shift);
	...
	public void add(String originator, ComparablePair<Date, Date> shift) {
		if (!shiftsPerOriginator.containsKey(originator)) {
			shiftsPerOriginator.put(originator, new TreeSet<ComparablePair<Date,Date>>());
		}
	...
	public String toString() {
		...
		for (Map.Entry<String, Set<ComparablePair<Date, Date>>> shift : shiftsPerOriginator.entrySet()) {
	...
	public TableModel toTableModel() {
		...
		ComparablePair <Date, Date>[] columnWithAllShifts = new ComparablePair [shifts.size()]; 
		...
		for(String originator: shiftsPerOriginator.keySet()){
			...
			Set<ComparablePair<Date, Date>> shiftsForThisOriginator = shiftsPerOriginator.get(originator);
			...
			tableShiftsByOriginator.addColumn(originator, columnWithShiftsPerOriginator);
