...
public abstract class StringDistanceMetric extends DistanceMetric {
	...
	/* (non-Javadoc)
	 * @see org.processmining.analysis.traceclustering.distance.DistanceMetric#getDistance(org.processmining.analysis.traceclustering.model.InstancePoint, org.processmining.analysis.traceclustering.model.InstancePoint)
	 */
	@Override
	...
	public double getDistance(LogEvents events, AuditTrailEntryList ateListA, AuditTrailEntryList ateListB) {
	...
	public DistanceMatrix getDistanceMatrix(LogReader log) {
		...
		FloatDistanceMatrix matrix = new FloatDistanceMatrix(log.numberOfInstances());
		for(int x=0; x<log.numberOfInstances()-1; x++) {
//			matrix.set(x, x, 0);
			AuditTrailEntryList ateListA = log.getInstance(x).getAuditTrailEntryList();
			for(int y=x+1; y<log.numberOfInstances(); y++) {
				//System.out.println("computing distance " + log.getInstance(x).getName() + " <-> " + log.getInstance(y).getName());
				AuditTrailEntryList ateListB = log.getInstance(y).getAuditTrailEntryList();
	...
	/* (non-Javadoc)
	 * @see org.processmining.analysis.traceclustering.distance.DistanceMetric#getDistanceMatrix(org.processmining.analysis.traceclustering.profile.Profile)
	 */
	@Override
