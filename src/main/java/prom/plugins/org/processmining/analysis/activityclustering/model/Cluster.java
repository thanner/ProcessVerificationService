...
public class Cluster implements Comparable<Cluster> {
	...
	public Cluster(LogReader log, int instanceIndex, FootprintReference footprintReference) {
		this.log = log;
	...
	public void addEvent(int index) throws IndexOutOfBoundsException, IOException {
		...
		footprint.add(log.getInstance(processInstanceIndex).getAuditTrailEntryList().get(index).getElement());
		isSorted = false;
	}
	
	public AuditTrailEntry getEvent(int index) throws IndexOutOfBoundsException, IOException {
		ensureSorted();
		return log.getInstance(processInstanceIndex).getAuditTrailEntryList().get(eventIndices.get(index));
	}
