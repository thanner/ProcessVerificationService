...
/**
 * Fitness evaluates whether the observed process <i>complies with</i> the control flow specified by the process. 
 * One way to investigate the fitness is to replay the log in the Petri net. 
 * The log replay is carried out in a non-blocking way, i.e., if there are tokens missing 
 * to fire the transition in question they are created artificially and replay proceeds. 
 * While doing so, diagnostic data is collected and can be accessed afterwards. 
 * The log replay can deal with invisible and duplicate tasks. 
 * <br>
 * The token-based <b>fitness</b> metric <i>f</i> relates the amount of missing tokens during log replay with the amount of 
 * consumed ones and the amount of remaining tokens with the produced ones. If the log could be replayed correctly, that is, 
 * there were no tokens missing nor remaining, it evaluates to 1. 
 * Similarly, if every produced and consumed token is remaining or missing, it evaluates to 0.
 * <br>
 * Assumptions: for the log replay it is assumed that the model has a unique start and end place, and that there are no 
 * dead tasks (i.e., it is a Workflow Net). Furthermore, it is assumed that the tasks in the given Petri net and the 
 * reference log are mapped to each other via their log event type. 
 * 
 * @author Anne Rozinat
 *
 */
public class TokenFitnessMetric implements BenchmarkMetric {
	...
	/**
	 * Configuration object telling the log replay that it has to collect data needed for 
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.benchmark.BenchmarkMetric#measure(org.processmining.framework.models.petrinet.PetriNet, org.processmining.framework.log.LogReader, org.processmining.framework.models.petrinet.PetriNet)
	 */
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel, Progress progress) {
		...
		LogReplayAnalysisMethod logReplayAnalysis = new LogReplayAnalysisMethod(model, referenceLog, new ConformanceMeasurer(), progress);
		int maxSearchDepth = MaximumSearchDepthDiagnosis.determineMaximumSearchDepth(model);
		logReplayAnalysis.setMaxDepth(maxSearchDepth); // automatically set maximum search depth for log replay
		ConformanceLogReplayResult replayResult = (ConformanceLogReplayResult) logReplayAnalysis.analyse(myOptions);
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.benchmark.BenchmarkMetric#needsReferenceLog()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see org.processmining.analysis.benchmark.BenchmarkMetric#needsReferenceModel()
	 */
	...
	/**
	 * Creates the configuration object that is used by the log replay to determine 
