...
public class PerformanceProfile extends AbstractProfile {
	 ...
	 */
	public PerformanceProfile(LogReader log) 
		...
		for(int c=0; c<log.numberOfInstances(); c++) {
			...
			for(int i=0; i<ateList.size(); i++) {
				...
				if(current != null) {
					...
					if(last != null) {
						...
						double time = new Double(current.getTime() - last.getTime());
						time /= 1000.0;
						// adjust mean
			...
			if(transitionCounter > 0 && maxTransitionTime > 0.0) {
				...
				if(transitionList.size() > 0) {
					...
					double medianTransitionTime = transitionList.get(medianIndex);
	...
	/**
	 * helper method: measures the total duration of a case 
	 * (zero if no timestamps present) in milliseconds
	 * @param ateList
	 * @return
	 * @throws IndexOutOfBoundsException
	 * @throws IOException
