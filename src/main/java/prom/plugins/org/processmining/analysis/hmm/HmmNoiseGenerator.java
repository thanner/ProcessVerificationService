...
public abstract class HmmNoiseGenerator {
	...
	protected String typeNoiseFolder; // where to put the logs with varying degrees of noise
	 ...
	 * @param anHmm the hmm to be used as a basis for generating the noisy logs
	 * @param noNoiseLevels the number of noise levels (without the 0% level). Should be between 1 and 100
	 * @param noTraces the number of traces that should be generated for each of the noise levels
	 * @param traceLength the maximum trace length per generated instance (in the presence of loops might be otherwise infinite)
	 * @param eventMapping the observation element mapping matching the given hmm
	 */
	public HmmNoiseGenerator(Map<String,PetriNet> models, HmmExpConfiguration aConf) {
		inputModels = models;
		conf = aConf;
	...
	/**
	 * Generates either a single log or replicated logs (depending on
	 * the user configuration) for the current noise level. 
	 * @param currentNoiseLevel the current noise level (between 0 and 100)
	 */
	...
	/**
	 * Generates an MXML event log based noisy HMM and stores
	 * file on disk while encoding the current noise level in the file name.
	 * @param noiseLevel the current noise level (between 0 and 100)
	 */
	protected void generateLog(double noiseLevel) {
		List<List<ObservationInteger>> sequences = generateSequences();
		File outputFile = new File(typeNoiseFolder, noiseLevel + "_PerCent.mxml.gz");
		writeLog(sequences, noiseLevel, outputFile);
	}

	/**
	 * Generates replicated MXML event logs and stores the files on disk 
	 * in a folder encoding the current noise level in the directory name. 
	 * file on disk while encoding the current noise level in the file name.
	 * @param noiseLevel the current noise level (between 0 and 100)
	 */
	private void generateReplicatedLog(double noiseLevel) {
		...
		for (int j=1; j<conf.getReplications() + 1; j++) {
			...
			File outputFile = new File(typeNoiseFolder + "/" + noiseLevel + "_PerCent", "Replication_" + j + ".mxml.gz");
			writeLog(sequences, noiseLevel, outputFile);
		}
	}
	 ...
	 * To be specified by each noise type in deriving sub class
	 */
	protected abstract void adjustHmm(int noiseInterval);
	
	/**
	 * Writes the current Hmm to file while encoding the current noise level in the file name.
	 * @param noiseLevel the  current noise level (between 0 and 100)
	 */
	protected void writeHmm(double noiseLevel) {
		try {
			(new GenericHmmDrawerDot()).write(noisyHmm, noiseHmmFolder + "/" + noiseLevel + "_PerCent.dot", observationMapping);
		} catch (Exception ex) {
			Message.add("Error writing the HMM to Dot file at noise level " + noiseLevel, Message.ERROR);
			ex.printStackTrace();
		}
	}
	 ...
	 * @param sequences the log traces as HMM sequences
	 * @param noiseLevel the current noise level
	 * @param outputFile the output file to write the log to
	 */
	private void writeLog(List<List<ObservationInteger>> sequences,
		...
		for (int j=0; j< sequences.size(); j++) {
			...
			persistency.startProcessInstance(name, description, new HashMap<String,String>());
			for(int k=0; k<currentSeq.size(); k++) {
				...
				if (observation != observationMapping.size()) {
					...
					ate.setWorkflowModelElement(event.getModelElementName());
					ate.setEventType(EventType.COMPLETE);
					persistency.addAuditTrailEntry(ate);
				}
				else {
					break; // once null observations are thrown this final state is not left anymore
				}
			}
		...
		persistency.finish();
			...
			Message.add("Error while writing log at noise level " + noiseLevel, Message.ERROR);
			ex.printStackTrace();
		}
	}
	...
	protected List<List<ObservationInteger>> generateSequences() {
		MarkovGenerator<ObservationInteger> mg = new MarkovGenerator<ObservationInteger>(noisyHmm);
