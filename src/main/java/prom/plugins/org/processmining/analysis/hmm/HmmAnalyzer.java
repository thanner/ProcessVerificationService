...
/**
 * Class realizing the mapping from Simple Petri nets to HMMs.
 * After creation the HMM-based metrics can be requested via 
 * {@link #getMetrics()}.
 */
public class HmmAnalyzer {
	...
	protected List<List<ObservationInteger>> nonDuplicateSequences; // traces without frequencies
	protected HashSet<LogEvent> alreadyProcessed;
	
	protected int successorFrequencies[]; // total number of observations following i
	...
	/**
	 * Returns the mapping of observation code symbols to actual 
	 * log events (manifested by their natural position in the list). 
	...
	/**
	 * Creates an HMM based on the given Petri net model. 
	 * Assumes that there is no parallelism, no duplicate tasks, 
	 * and a clear start place. 
	 */
	protected Hmm<ObservationInteger> createHmm() {
		...
		OpdfIntegerFactory factory = new OpdfIntegerFactory(data.observed.size() + 1);
		Hmm<ObservationInteger> resultHmm = new Hmm<ObservationInteger>(data.observed.size() + 1, factory); // plus final state
		...
		for (int i=0; i<data.observed.size() + 1; i++) { // final state has dummy observation
		...
		for (Transition fin : finalStates) {
			...
			// adjust the transition probabilities to the successors (lower as also possible to end)
			for (Transition finSuc : allFinSuccessors) {
				int finSucIndex = data.observed.indexOf(finSuc.getLogEvent());
				resultHmm.setAij(finIndex, finSucIndex, (1.0 / (double) (allFinSuccessors.size() + 1)));
			}
			resultHmm.setAij(finIndex, data.observed.size(), (1.0 / (double) (allFinSuccessors.size() + 1)));
		}
	...
	protected ArrayList<Transition> getNextVisibleTransitions(Transition trans) {
		...
		if (trans.outDegree() > 0) {
			Edge outEdge = trans.getOutEdges().get(0); // assume sequential models
	...
	protected ArrayList<Transition> getPreviousVisibleTransitions(Transition trans) {
		ArrayList<Transition> resultList = new ArrayList<Transition>();
		if (trans.inDegree() > 0) {
			...
			if (inPlace.inDegree() > 0) {
				for (Edge prevEdge : inPlace.getInEdges()) {
					...
					if (prevTrans.isInvisibleTask() == false) {
						...
						resultList.addAll(getPreviousVisibleTransitions(prevTrans));
					}
				}
			}
		}
	...
	protected void traceTransition(Transition trans, Hmm<ObservationInteger> hmm) {
		if (alreadyProcessed.contains(trans.getLogEvent()) == false) {
			...
			for (Transition nextTrans : nextTransitions) {
				...
				hmm.setAij(transIndex, nextIndex, 1.0 / ((double) nextTransitions.size()));
	...
	protected void createObservationSequences() {
		try {
			data.eventTraceFrequencies = new int[inputLog.getLogSummary().getNumberOfUniqueProcessInstances()][data.observed.size()][data.observed.size()];
			successorFrequencies = new int[data.observed.size()]; // how often i was succeeded by anything (needed to normalize transition probabilities in hmm later)
			data.traceFrequencies = new int[inputLog.getLogSummary().getNumberOfUniqueProcessInstances()];
			// initialize
			for (int i=0; i<data.observed.size(); i++) {
				successorFrequencies[i] = 0;
				for (int j=0; j<data.observed.size(); j++) {
					for (int k=0; k<inputLog.getLogSummary().getNumberOfUniqueProcessInstances(); k++) {
			...
			for (int i=0; i<inputLog.numberOfProcesses(); i++) {
				...
				for (int j=0; j<process.size(); j++) {
					...
					int previousIndex = -1;
					noGroupedInstances = MethodsForWorkflowLogDataStructures.getNumberSimilarProcessInstances(instance);
					...
					for (int k=0; k<ateList.size(); k++) {
						...
						LogEvent le = inputLog.getLogSummary().getLogEvents().findLogEvent(current.getElement(), current.getType());
						int leIndex = data.observed.indexOf(le);
						currentSequence.add(new ObservationInteger(leIndex));
						// also store actual frequencies to adjust hmm parameters
						if (previousIndex != -1) {
							data.eventTraceFrequencies[j][previousIndex][leIndex]++;
							successorFrequencies[previousIndex] = successorFrequencies[previousIndex] + noGroupedInstances;
						}
	...
	protected void evaluateObservationSequences(Hmm<ObservationInteger> hmm) {
		...
		for (List<ObservationInteger> seq : nonDuplicateSequences) {
			...
			System.out.println("Sequence probability " + i + ": " + currentProb);
			sumProb = sumProb + currentProb;
			i++;
		}
		System.out.println("Total sum of observation probabilities: " + sumProb);
		// calculate most likely state sequence according to viterbi
		i = 0;
		for (List<ObservationInteger> seq : nonDuplicateSequences) {
			ViterbiCalculator viterbi = new ViterbiCalculator(seq, hmm);
			System.out.println("Most likely states for observation " + i + ": "); 
	...
	protected void adjustHmm() {
		...
		data.falseNegatives = new boolean[data.observed.size()][data.observed.size()];
		data.falsePositives = new boolean[data.observed.size()][data.observed.size()];
		for (int i=0; i<data.observed.size(); i++) {
			for (int j=0; j<data.observed.size(); j++) {
				double modelProb = adjustedHmm.getAij(i, j);
				double actualProb = 0.0;
				if (successorFrequencies[i] != 0) {
					//actualProb = (double) data.actualFrequencies[i][j] / (double) successorFrequencies[i];
					for (int k=0; k<inputLog.getLogSummary().getNumberOfUniqueProcessInstances(); k++) {
						actualProb = actualProb + ((double) data.eventTraceFrequencies[k][i][j] * data.traceFrequencies[k]);
					}
	...
	public Hmm<ObservationInteger> createRandomHmm() {
		...
		for (int i=0; i<data.observed.size(); i++) {
			for (int j=0; j<data.observed.size() + 1; j++) {
				resultHmm.setAij(i, j, (1.0 / ((double) (data.observed.size() + 1))));
			}
		}
