...
/**
 * Displays the high level information for the provided
 * process in a generic way.
 *
 * @author Anne Rozinat
 */
public class EditHighLevelProcess implements AnalysisPlugin {
	...
	public AnalysisInputItem[] getInputItems() {
		AnalysisInputItem[] items = { new AnalysisInputItem("High Level Process") {
			public boolean accepts(ProvidedObject object) {
				Object[] o = object.getObjects();
				for (int i = 0; i < o.length; i++) {
                                    if (o[i] instanceof HLModel ||
                                        o[i] instanceof HLProcess ||
	...
	public JComponent analyse(AnalysisInputItem[] analysisInputItemArray) {
		...
		for (int i = 0; i < o.length; i++) {
			...
			// for convenience: allow to create high-level versions of plain control-flow
			// models without the need to call one of the analysis plugins that creates one
			} else if (o[i] instanceof PetriNet) {
				hlProcessGui = new EditHighLevelProcessGui(new HLPetriNet((PetriNet) o[i]));
			} else if (o[i] instanceof YAWLModel) {
				hlProcessGui = new EditHighLevelProcessGui(new HLYAWL((YAWLModel) o[i]));
                                //added by Mariska Netjes
                        } else if (o[i] instanceof ProtosModel) {
				hlProcessGui = new EditHighLevelProcessGui(new HLProtos((ProtosModel) o[i]));
                        }
		}
		return hlProcessGui;
	}
