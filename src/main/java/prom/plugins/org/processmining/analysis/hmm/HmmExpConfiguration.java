...
/**
 * Configuration class holding the parameters of the experiment
 * to be performed. 
 */
public class HmmExpConfiguration {
	
	public enum BaseLine{ MODEL, LOG }
	
	private static HmmExpConfiguration singleton = new HmmExpConfiguration();
	private int noiseLevels = 20; // between 1 and 100 (additional log without noise is implied)
	private int traces = 100; // number of traces to be generated per noise level (at least one)
	private int traceLength = 100; // maximum number of events per trace
	private BaseLine referencePoint; // whether to use model-based or log-based Hmm for noise generation
