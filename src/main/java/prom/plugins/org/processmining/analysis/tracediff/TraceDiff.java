...
/**
 * Class mapping the differences obtained from the diff operation to 
 * audit trail entries in the given trace. Interpretation depends on 
 * the side of the comparison in diff ('to' or 'from'). 
 * 
...
public abstract class TraceDiff {
	...
	protected int[] mapped; // the original indicees mapped to the global indicees
	protected boolean[] diffed; // the original indicees whether part of diff (true) or not (false)
	protected int[] reverse; // the global indicess mapped to the original indicees (-1 if not present)
	protected ArrayList<Integer> diffs; // the start of diffs in terms of global indices
	 ...
	 * @param index the global position
	...
	/**
	 * Tests whether the original event at the given index 
	 * was part of a difference in the diff.
	 * @param index the global position to be checked
	 ...
	 * @param origIndex the index in the undiffed trace
	...
	/**
	 * Retrieve the original event index for the global 
	 * position.
	...
	/**
	 * Builds the reverse mapping from global indecees to orginal trace indicees. 
	 */
	public void buildReversedMapping(int maxIndex) {
		// get mapped index of last ate and add 1 to get the size (since starting from 0)
	 ...
	 * @param difference the difference for which the start is to be determined
	 ...
	 * @param difference the difference for which the end is to be determined
	...
	/**
	 * Obtains the offset between the changes made in the one original trace and the 
	 * other original trace
	 * @param difference the difference for which the offset is to be determined
	 * @return 0 if equal or less changes are made in this trace compared to the 
	 * other trace, a number > 0 if more elements are affected in this tracen than 
	 * in the other trace. 
	 */
