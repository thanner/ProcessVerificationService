...
/**
 * The structural appropriateness metric evaluates the state space of the model and checks for 
 * both redundant invisible and alternative duplicate tasks. 
 * <br>
 * Values range from 0 (all task in model are either alternative duplicate or redundant invisible) 
 * to 1 (none of them is).
 * <br>
 * Assumptions: Alternative duplicate tasks in loops are not detected. Furthermore, invisible tasks that
 * are redundant only with lazy semantics are also not detected yet. 
 * 
 * @author Anne Rozinat
 */
public class StructuralAppropriatenessMetric implements BenchmarkMetric {
	...
	public double measure(PetriNet model, LogReader referenceLog, PetriNet referenceModel, Progress progress) {
		...
		StructuralAnalysisMethod structuralAnalysis = new StructuralAnalysisMethod(model);
		StructuralAnalysisResult structuralResult = (StructuralAnalysisResult) structuralAnalysis.analyse(myOptions);
		...
		// return result
		double structuralAppropriateness = stateSpaceResult.getImprovedStructuralAppropriatenessMeasure();
