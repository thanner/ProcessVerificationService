 ...
 * based on the structure of the Petri net. 
 * This can be used to build only a partial state space during the search for 
 * invisible tasks that might enable another, visible task. 
 * The maximum number of directly connected invisible tasks is determined for 
...
public class MaximumSearchDepthDiagnosis {
	...
	/**
	 * This method estimates the maximum search depth based on the maximum number 
	 * of directly connected invisible tasks. 
	...
	public static int determineMaximumSearchDepth(PetriNet net) {
		...
		while (allTrans.hasNext()) {
			...
			int currentMax = MaximumSearchDepthDiagnosis.determineMaximumEnablingSequ(trans, new ArrayList<Transition>());
			if (currentMax == -1) {
				// due to loop of invisible tasks no maximum search depth can be determined
				return -1;
			...
			if (place.outDegree() == 0) {
				...
				while (inEdgesFinalPlace.hasNext()) {
					...
					if (trans.isInvisibleTask() == true) {
						...
						int currentMax = MaximumSearchDepthDiagnosis.determineMaximumEnablingSequ(trans, alreadyVisited);
						if (currentMax == -1) {
							// due to loop of invisible tasks no maximum search depth can be determined
							return -1;
	...
	/**
	 * Helper method to determine the maximum enabling sequence of invisible tasks
	 * from the given transition backwards. 
	 * @param trans the transition for which to determine the max sequence
	 * @param alreadyVisited the list of invisible transitions already traced in recursive way. Needed 
	 * to prevent infinite loop in case of loop of invisible tasks in the model
	 * @return the maximum enabling sequence of invisible tasks for this task. Can be 0 if there are no invisible tasks 
	 * in the model. If there are loops of invisible tasks then the whole coverability graph needs to be built during log replay 
	 * (and now maximum search depth can be determined). In this case -1 is returned 
	 */
	private static int determineMaximumEnablingSequ(Transition startTrans, ArrayList<Transition> alreadyVisited) {
		int max = 0;
				...
				while (inEdgesPlace.hasNext()) {
					...
					if (trans.isInvisibleTask() == true) {
						...
						if (alreadyVisited.contains(trans)) {
							...
							alreadyVisited.add(trans);
							int transMax = MaximumSearchDepthDiagnosis.determineMaximumEnablingSequ(trans, alreadyVisited);
							// check for loops of invisible tasks
							if (transMax == -1) {
								return -1;
							} else if (transMax + 1 > placeMax) {
								// set current transition to current maximum (plus 1 as the invisible task itself needs to be counted)
								placeMax = transMax + 1;
							}
						}
					}	
