...
/**
 * List model building up the diff of two traces in the same list. 
...
public class DiffedAuditTrailEntryListModel extends AbstractListModel {
	 ...
	 * @param left the 'from' trace in the comparison
	 * @param right the 'to' trace in the comparison
	 * @param aDiff the diff object from the longest subsequence analysis
	 */
	public DiffedAuditTrailEntryListModel(AuditTrailEntryList left, AuditTrailEntryList right, Diff aDiff) {
		leftTrace = left;
		rightTrace = right;
		diff = aDiff;
		leftDiff = new LeftTraceDiff(leftTrace, diff);
		rightDiff = new RightTraceDiff(rightTrace, diff);
		maxIndex = Math.max(leftDiff.getGlobalIndex(leftTrace.size() - 1), rightDiff.getGlobalIndex(rightTrace.size() - 1));
	...
	public Object getElementAt(int index) {
		return new EventDiff(leftDiff.getElementAt(index),
				rightDiff.getElementAt(index),
				leftDiff.elementDiffed(index),
				rightDiff.elementDiffed(index),
				leftDiff.getOriginalIndex(index),
