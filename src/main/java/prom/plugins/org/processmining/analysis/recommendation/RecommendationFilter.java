 ...
 * <p>Empty query filters, or filter fields, are ignored gracefully.
 *
...
public class RecommendationFilter {
	 ...
	 * @param query The recommendation query containing the filter specification.
	 * @param result The recommendation result to be filtered.
	 * @return The filtered recommendation result (containing only recommendations which
	 * have passed the query's filter).
	 */
	public static RecommendationResult filter(RecommendationQuery query,
			RecommendationResult result) {
		RecommendationResult filteredResult = new RecommendationResult(query.getId(),result.getPartialTraceValue());
	 ...
	 * query's filter.
	 * @param query The query containing the filter attributes.
	 * @param rec Recommendation to be filtered.
	...
	public static boolean accept(RecommendationQuery query, Recommendation rec) {
		...
		// group, role, or user specified must overlap between query and recommendation
		...
		if (query.getFilterGroups() != null && rec.getGroups() != null) {
			for (String group : rec.getGroups()) {
				if (query.getFilterGroups().contains(group)) {
					// one of the recommended groups should go through the filter
