 ...
 * into a high-level Petri net model.
 * <br>
 * This means that the control-flow structure is converted while the
 * high-level information is preserved.
 *
 * @author Anne Rozinat
 */
public class HLYAWLToHLPetriNet implements ConvertingPlugin {
	...
	public MiningResult convert(ProvidedObject object) {
		...
		for (int i = 0; i < object.getObjects().length; i++) {
			if (providedYAWL == null && object.getObjects()[i] instanceof HLYAWL) {
	 ...
	 * @param hlYAWL the given input model
	...
	protected HLPetriNet convert(HLYAWL hlYAWL) {
	    ...
	    YAWLModel model1 = YawlToYawl.addImplicitConditions(yawlModel, settings0, map0);
	    PetriNet petriNetResult = YawlToPetriNet.convert(model1, settings1, map1);
		...
		for (ModelGraphVertex actVert : hlYAWL.getActivityNodes()) {
			...
			if (mappedVert instanceof YAWLTask) {
				for (Object pnVert : (ArrayList) map1.get(mappedVert)) {
					if (pnVert instanceof Transition && ((Transition) pnVert).isInvisibleTask() == false) {
						// look for the visible transition among the mapped objects in the list
						// as the corresponding activity node in the PN model
						yawlPnActivityMapping.put((YAWLTask) actVert, (Transition) pnVert);
					}
					if (((YAWLTask) mappedVert).getSplitType() == EPCConnector.XOR &&
							pnVert instanceof Place) {
						// look for the place that corresponds to the choice node for this explicit
						// split task
						yawlPnChoiceMapping.put((YAWLTask) actVert, (Place) pnVert);
					}
				}
			}
		}
		for (ModelGraphVertex actVert : hlYAWL.getChoiceNodes()) {
			...
			if (mappedVert instanceof YAWLCondition) {
				...
				// look for deferred choice constructs, which are directly mapped onto PN places
				yawlPnChoiceMapping.put((YAWLCondition) mappedVert, pnPlace);
			}
		}

		// TODO: check whether to remove event type from transition name already in newYAWL import
		Iterator<Transition> transitions = petriNetResult.getTransitions().iterator();
		...
		HLPetriNet hlPetriNet = makeHLPetriNet(hlYAWL, petriNetResult, yawlPnActivityMapping, yawlPnChoiceMapping);
	 ...
	 * @param hlYAWL the source HLYAWL model
	 * @param petriNetResult the converted low-level Petri net model
	 * @param yawlPnActivityMapping the mapping from YAWL task nodes to PN transitions
	 * @param yawlPnChoiceMapping the mapping from YAWL choice nodes to PN places
	 * @return the created target HLPetriNet model
	 */
	private HLPetriNet makeHLPetriNet(HLYAWL hlYAWL, PetriNet petriNetResult,
			HashMap<YAWLTask, Transition> yawlPnActivityMapping, HashMap<YAWLNode, Place> yawlPnChoiceMapping) {
		...
		for (ModelGraphVertex graphNodeYAWL : hlYAWL.getActivityNodes()) {
			for (ModelGraphVertex graphNodePN : hlPetriNet.getGraphNodes()) {
				if (graphNodeYAWL instanceof YAWLTask  && yawlPnActivityMapping.get(graphNodeYAWL) == graphNodePN) {
					HLActivity clonedAct = (HLActivity) hlYAWL.findActivity(graphNodeYAWL).clone();
		...
		for (ModelGraphVertex graphNodeYAWL : hlYAWL.getChoiceNodes()) {
			for (ModelGraphVertex graphNodePN : hlPetriNet.getGraphNodes()) {
				if ((graphNodeYAWL instanceof YAWLCondition && graphNodeYAWL.outDegree() > 1) ||
						(graphNodeYAWL instanceof YAWLTask && ((YAWLTask)graphNodeYAWL).getSplitType() == EPCConnector.XOR && yawlPnChoiceMapping.get(graphNodeYAWL) == graphNodePN)) {
					HLChoice clonedCh = (HLChoice) hlYAWL.findChoice(graphNodeYAWL).clone();
	...
	/**
	 * Replaces the condition targets for choices that stem from an XOR split task.
	 * <p>Instead of taking the succeeding YAWL tasks as a choice target, the newly added
	 * invisible split transitions belonging to this XOR split are assigned as the condition targets.
	 * <p>This is needed if the process should be simulated and the choice should be made based on a
	 * probability or data condition. Otherwise pre-condition might lead to deadlock situation for
	 * explicit choices. Note that deferred choices are preserved in the way they are in the HLYAWL model.
	 * @param yawlPnChoiceMapping the mapping of YAWL choice nodes to PN places
	 * @param hlPetriNet the target high-level Petri net (resulting from this conversion)
	 */
	private void updateConditionTargets(HashMap<YAWLNode, Place> yawlPnChoiceMapping, HLPetriNet hlPetriNet) {
				...
				for (Edge edge : place.getOutEdges()) {
					...
					if (newTarget == null) {
						Message.add("Error during adaptation of transition targets. Target " +
								trans.getIdentifier() + " not found", Message.ERROR);
						return;
					}
					// XOR split transitions should only have one output place
					assert(trans.outDegree() == 1);
					Place outPlace = (Place) trans.getOutEdges().get(0).getHead();
					...
					for (Edge targEdge : outPlace.getOutEdges()) {
						...
						if (targTrans.isInvisibleTask() == true) {
							// XOR join transitions should only have one output place
							assert(targTrans.outDegree() == 1);
							Place newOutPlace = (Place) targTrans.getOutEdges().get(0).getHead();
							assert(newOutPlace.outDegree() == 1);
							targTrans = (Transition) newOutPlace.getOutEdges().get(0).getHead();
						}
						HLID actID = hlPetriNet.getActivityID(targTrans);
						if (actID != null) {
							choice.replaceChoiceTarget(actID, newTarget.getID());
						} else {
							Message.add("Error during adaptation of transition targets. Target " +
									targTrans.getIdentifier() + " not found", Message.ERROR);
						}
					}
				}
			}
	...
	/**
	 * Copies the high-level information from the HLYAWL model to the target HLPetriNet.
	 * @param hlYAWL the source of the high-level information to be copied
	...
	private void copyHighLevelInformation(HLYAWL hlYAWL, HLPetriNet hlPetriNet) {
		hlPetriNet.getHLProcess().getGlobalInfo().setName(hlYAWL.getHLProcess().getGlobalInfo().getName());
		hlPetriNet.getHLProcess().getGlobalInfo().setTimeUnit(hlYAWL.getHLProcess().getGlobalInfo().getTimeUnit());
		hlPetriNet.getHLProcess().getGlobalInfo().setCaseGenerationScheme(hlYAWL.getHLProcess().getGlobalInfo().getCaseGenerationScheme());
		Iterator<HLAttribute> attributes = hlYAWL.getHLProcess().getAttributes().iterator();
		while (attributes.hasNext()) {
			...
			Iterator<HLActivity> actWithInputIt = hlYAWL.getHLProcess().getActivitiesForInputAttribute(att.getID()).iterator();
			while (actWithInputIt.hasNext()) {
				HLActivity yawlAct = actWithInputIt.next();
				HLActivity pnAct = hlPetriNet.getHLProcess().getActivity(yawlAct.getID());
				if (pnAct != null) {
					pnAct.addInputDataAttribute(att.getID());
				} else {
					Message.add("Failure while reconnecting high-level information: attribute " + att.getName() +
							" could not be assigned to activity " + yawlAct.getName() + ". Activity ID in YAWL model: " +
							yawlAct.getID().toString() + ".", Message.ERROR);
				}
			}
                        // also add output attribute to activities
                        Iterator<HLActivity> actWithOutputIt = hlYAWL.getHLProcess().getActivitiesForOutputAttribute(att.getID()).iterator();
		...
		Iterator<HLResource> resources = hlYAWL.getHLProcess().getResources().iterator();
		...
		hlPetriNet.getHLProcess().setNobodyGroupID(hlYAWL.getHLProcess().getNobodyGroupID());
		hlPetriNet.getHLProcess().removeGroupWithoutAffectingAssigmnets(hlPetriNet.getHLProcess().getAnybodyGroupID());
		hlPetriNet.getHLProcess().setAnybodyGroupID(hlYAWL.getHLProcess().getAnybodyGroupID());
		// now clone the real group objects
		Iterator<HLGroup> groups = hlYAWL.getHLProcess().getAllGroups().iterator();
		while (groups.hasNext()) {
			HLGroup grp = groups.next();
			HLGroup clonedGrp = (HLGroup) grp.clone();
			hlPetriNet.getHLProcess().addOrReplace(clonedGrp);
		}
		Iterator<HLTypes.Perspective> perspectives = hlYAWL.getHLProcess().getGlobalInfo().getPerspectives().iterator();
