...
public class FuzzyModelCreator {
	...
	public void addEdge(String sourceName, String sourceType, 
			String targetName, String targetType, double significance, double correlation) {
	...
	public void addEdge(LogEvent source, LogEvent target, double significance, double correlation) {
		addEdge(source.getModelElementName(), source.getEventType(), 
				target.getModelElementName(), target.getEventType(), significance, correlation);
	}
	
	public void addEdge(String sourceName, String targetName, double significance, double correlation) {
		addEdge(sourceName, "complete", targetName, "complete", significance, correlation);
	}
	...
	public MutableFuzzyGraph generateGraph() {
		ensureMetricsCreated();
		return new MutableFuzzyGraph(nodeSignificance, edgeSignificance, edgeCorrelation, events);
	}
	...
	protected void ensureMetricsCreated() {
		if(nodeSignificance == null) {
			...
			nodeSignificance = new UnaryMetric("Unary significance", "Node significance", size);
			for(int i=0; i<size; i++) {
				nodeSignificance.setMeasure(i, eventSignificances.get(i));
			}
			edgeSignificance = new BinaryMetric("Binary significance", "Edge significance", size);
			edgeCorrelation = new BinaryMetric("Binary correlation", "Edge correlation", size);
		}
	}
