...
public class PostProcessOfUML2SequenceChartImport implements ConvertingPlugin {
	...
	private HLPetriNet generateGuards(PetriNet petriNet, LogReader logReader) {
		...
		for (HLChoice choice : hlpn.getHLProcess().getChoices()) {
			for (Transition transition : PetriNetNavigation.getOutgoingTransitions(hlpn.findModelGraphVertexForChoice(choice.getID()))) {
				...
				if (guard != null) {
					...
					// TODO: instead of setting CPN guard string, provide the expression
					// as expression object (can be converted to CPN-compliant string from there)
