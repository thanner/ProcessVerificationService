...
public class EPC2FuzzyModel implements ConvertingPlugin {
	...
	protected void addAllSuccessorEdges(ConfigurableEPC epc, FuzzyModelCreator kreator, 
			EPCFunction source, Node start, boolean forceFollow) {
		if(start.getOutEdges() != null && (forceFollow || source.equals(start) == false)) {
			for(Edge outEdge : start.getOutEdges()) {
				Node target = outEdge.getHead();
				if(target == null) {
					continue;
				} else if(target instanceof EPCFunction && ((EPCFunction)target).getLogEvent() != null) {
					kreator.addEdge(source.getLogEvent(), ((EPCFunction) target).getLogEvent(), 1.0, 1.0);
				} else {
