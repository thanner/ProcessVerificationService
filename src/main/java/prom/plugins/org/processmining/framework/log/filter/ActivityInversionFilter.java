...
//		}
			...
			while (index < entries.size()) {
				...
				if (current.getType().equals("complete")) {
					...
					if (index + 1 < entries.size()) {
						...
						// case 1: "complete-to-start" becomes "start-to-complete"
						if (next.getType().equals("start")) {
							newName = current.getElement() + "-to-" + next.getElement();
						...
						// case 2: "complete-to-complete" becomes "start-to-complete"
					...
					// if the log is indeed formed by succeeding 'start' and 'end' events, 
					// this should only happen at the very beginning and very end of each instance
	...
	public LogFilterParameterDialog getParameterDialog(LogSummary summary) {
		return new LogFilterParameterDialog(summary, ActivityInversionFilter.this) {
