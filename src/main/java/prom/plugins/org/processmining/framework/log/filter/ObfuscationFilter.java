...
//			}
			...
			try {
				...
				// TODO: make an option panel for this filter (offer to store mapping information optionally there)
				// open file to store mapping
				//JFileChooser saveDialog = new JFileChooser();
				//saveDialog.setSelectedFile(new File("Mapping_ObfuscationLogFilter_" + instance.getName() + ".txt"));
				//if(saveDialog.showSaveDialog(MainUI.getInstance())==JFileChooser.APPROVE_OPTION) {
				//	File outFile = saveDialog.getSelectedFile();
					try {
						/* Caesar algorithm after http://homepages.gold.ac.uk/rachel/Caesar.java. */
						int key=22; //I have chosen 22 to be my key value but you can change this to any value between 0 and 25
						...
						//		"% Offset: " + key + " (can be anything between 0 and 25) \n\n");
						...
						while (index < entries.size()) {
							...
							for(int i=0;i<s.length();i++)  //we look at the plaintext one character at a time
							{
								...
								// now we print out the encrypted character or if the character 
								// was not a letter then we print the original character.
						...
						while (it.hasNext()) {
							String clear = it.next();
							String obfuscated = mapping.get(clear);
							outWriter.write(clear + " --> " + obfuscated + "\n");
						}
						// finish the writing
						outWriter.flush();
						outWriter.close();*/
