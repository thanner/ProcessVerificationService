...
//		putAttribute(sysdfltGraphAttributes, SUBGRAPH, COLOR_ATTR, "lightskyblue1");
	}
	...
	public Object setGrappaAttribute(String key, String value) throws IllegalArgumentException {
		...
		if (oldValue == null) {
			if (!validGrappaAttributeKey(key)) {
				throw new IllegalArgumentException(Grappa.PKG_UPLOW + " attribute key must use \"" +
						Grappa.PKG_LOWER + "\" as a prefix");
			}
	...
	public static boolean validGrappaAttributeKey(String key) {
		return (key != null && key.startsWith(Grappa.PKG_LOWER) &&
				key.length() > Grappa.PKG_LOWER.length());
	}
	...
	static Long idMapKey(int type, int id) throws IllegalArgumentException {
		...
		value = (value << Grappa.TYPES_SHIFT) | (type & (Grappa.NODE | Grappa.EDGE |
				Grappa.SUBGRAPH));
