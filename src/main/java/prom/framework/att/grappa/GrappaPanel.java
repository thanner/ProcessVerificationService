...
public class GrappaPanel extends javax.swing.JPanel implements
		...
		if (!top) {
			...
			scrollPane.getHorizontalScrollBar().addAdjustmentListener(new AdjustmentListener() {
				public void adjustmentValueChanged(AdjustmentEvent e) {
					//Message.add("Horizontal scrollbar value changed", Message.DEBUG);
					repaintPreview();
				}
			});
			scrollPane.getVerticalScrollBar().addAdjustmentListener(new AdjustmentListener() {
				public void adjustmentValueChanged(AdjustmentEvent e) {
					//Message.add("Vertical scrollbar value changed", Message.DEBUG);
					repaintPreview();
				}
			});
		}
		...
		if (top) {
			...
			slider.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent e) {
					//	if ((slider.getValue() % slider.getMinorTickSpacing() == 0) ||
			...
			preview.setMinimumSize(new Dimension((int) slider.getMinimumSize().getWidth(),
					(int) slider.getMinimumSize().getWidth()));
			...
			split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, scrollPane,
							   rightPanel);
	...
	public GrappaListener addGrappaListener(GrappaListener listener) {
		...
		//PVDB following two lines added to solve bug with adding multiple Listeners to one instance
...
//		Message.add("painting"+paint_i++);
		...
		if (scaleToFit || scaleToSize != null) {
			...
			graphPanel.setPreferredSize(new Dimension((int) Math.ceil(scaleToWidth),
					(int) Math.ceil(scaleToHeight)));
			...
			Rectangle r = new Rectangle(scrollPane.getViewport().getViewPosition().x,
			...
			if (prnt != null) {
				...
				Point p = new Point(
						 Math.max(0, (int) (cpt.getX() - ((double) viewsize.width) / 2.)),
						 Math.max(0, (int) (cpt.getY() - ((double) viewsize.height) / 2.))
						  );
			...
			graphPanel.setPreferredSize(new Dimension((int) Math.ceil(bbox.getWidth()),
					(int) Math.ceil(bbox.getHeight())));
			transform.translate( -bbox.getMinX(), -bbox.getMinY());
		...
		synchronized (graph) {
			...
			if (grappaNexus != null) {
				...
				g2d.setPaint(bkgdColor = (Color) (graph.getGrappaAttributeValue(
...
//						g2d.setPaint(grappaNexus.style.line_color);
	...
	public String getToolTipText(MouseEvent mev) {
		...
		return (grappaListener.grappaTip(subgraph, findContainingElement(subgraph,
	...
	public GrappaBox zoomToOutline() {
		if (savedOutline != null) {
			...
			double hratio = Math.abs(((double) grappaGraphPanel.getWidth()) /
							(double) (realDragStartPoint.getX() - realDragEndPoint.getX()));
			double vratio = Math.abs(((double) grappaGraphPanel.getHeight()) /
							(double) (realDragStartPoint.getY() - realDragEndPoint.getY()));
			setScaleFactor(Math.min(hratio, vratio));
			scrollPane.getViewport().setViewPosition(
					new Point((int) (scaleFactor * Math.min(realDragStartPoint.getX(),
					realDragEndPoint.getX())),
					(int) (scaleFactor * Math.min(realDragStartPoint.getY(), realDragEndPoint.getY()))
					)
					);
			savedOutline = null;
		}
	 ...
	 * Adjust the scale factor by the supplied multiplier.
	 *
	 * @param factor set the scale factor to this amount.
	 * @return the value of the previous scale factor.
	 */
...
//			previewLabel.setText("Zoom: " + slider.getValue() + " %");
	...
	private void repaintPreview() {
		if ((preview != null) && (inverseTransform != null)) {
			Point2D tl = inverseTransform.transform(new Point(graphPanel.getVisibleRect().x,
						 graphPanel.getVisibleRect().y), null);
			Point2D wh = inverseTransform.transform(new Point(graphPanel.getVisibleRect().width +
						 graphPanel.getVisibleRect().x,
						 graphPanel.getVisibleRect().height +
						 graphPanel.getVisibleRect().y), null);
			GrappaBox box = new GrappaBox(tl.getX(), tl.getY(), (wh.getX() - tl.getX()),
							(wh.getY() - tl.getY()));
			preview.setRect(box);
			preview.repaint();
		}
	...
	private void paintSubgraph(Graphics2D g2d, Subgraph subg, Shape clipper, Color bkgdColor,
		...
		if (bbox != null && grappaNexus != null && subg.visible && !grappaNexus.style.invis &&
				clipper.intersects(bbox)) {
...
//						g2d.setPaint(grappaNexus.style.line_color);
	...
	private Element reallyFindContainingElement(Subgraph subg, Point2D pt, Element[] stash) {
		...
		if (bb.contains(pt)) {
			...
			enu = subg.subgraphElements();
			while (enu.hasMoreElements()) {
				if ((subelem = reallyFindContainingElement((Subgraph) (enu.nextElement()), pt,
							   stash)) != null) {
					if (stash[0] == null) {
						return (subelem);
					}
	...
	public void mouseClicked(MouseEvent mev) {
		...
		grappaListener.grappaClicked(subgraph, findContainingElement(subgraph,
				inverseTransform.transform(mev.getPoint(), null),
				(subgraph.currentSelection != null && subgraph.currentSelection instanceof Element ?
				((Element) subgraph.currentSelection) : null)), new GrappaPoint(pt.getX(), pt.getY()),
				mev.getModifiers(), mev.getClickCount(), this);
	}
	...
	public void mouseReleased(MouseEvent mev) {
		...
		double hgap = (pt.getX() - inverseTransform.transform(mev.getPoint(), null).getX());
		double vgap = (pt.getY() - inverseTransform.transform(mev.getPoint(), null).getY());

		GrappaBox gb = null;
		if (outline != null) {
			gb = new GrappaBox(outline.x - hgap, outline.y - vgap, outline.width,
				 outline.height);
		}
		grappaListener.grappaReleased(subgraph, findContainingElement(subgraph,
				inverseTransform.transform(mev.getPoint(), null)), gpt, modifiers,
				pressedElement, pressedPoint, pressedModifiers, gb, this);
		...
		if ((modifiers & java.awt.event.InputEvent.BUTTON1_MASK) != 0 &&
			...
			if (outline != null) {
				...
				savedOutline = GrappaSupport.boxFromCorners(outline, pressedPoint.x - offx,
							   pressedPoint.y - offy,
							   gpt.x - offx, gpt.y - offy);
				outline = null;
			} else {
	...
	public void recordDragPoints() {
		realDragStartPoint = new Point2D.Double(realPressedPoint.getX(), realPressedPoint.getY());
		realDragEndPoint = new Point2D.Double(realReleasedPoint.getX(), realReleasedPoint.getY());
	}
	...
	public void mouseDragged(MouseEvent mev) {
		...
		grappaListener.grappaDragged(subgraph, gpt, modifiers, pressedElement, pressedPoint,
				pressedModifiers, outline, this);

		if ((modifiers & java.awt.event.InputEvent.BUTTON1_MASK) != 0 &&
				(modifiers & java.awt.event.InputEvent.BUTTON1_MASK) == modifiers) {
			outline = GrappaSupport.boxFromCorners(outline, pressedPoint.x, pressedPoint.y, gpt.x,
					  gpt.y);
		}
	}
	...
	/**
	 * Always returns true as the viewport should  force the height of
	 * this GrappaPanel to match the height of the viewport.
	 *
	 * @return true
	 */
	public final boolean getScrollableTracksViewportHeight() {
	...
	class PreviewMouseHandler implements MouseListener, MouseMotionListener {
		...
		public void drawMain(MouseEvent e) {
			Point2D tl = parentOfPreview.transform.transform(
						 new GrappaPoint(
						 rectangle.getMinX(),
						 rectangle.getMinY()), null);
			Point p = new Point(
					 Math.max(0, (int) (tl.getX())),
					 Math.max(0, (int) (tl.getY()))
					  );
