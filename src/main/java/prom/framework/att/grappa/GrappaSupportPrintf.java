					...
					switch (ch = format[n++]) {
						case 'b':
							...
							if (args[argn] instanceof java.awt.geom.Rectangle2D) {
								cvt.buildBox(prtbuf, ((java.awt.geom.Rectangle2D) args[argn++]), false,
										flag);
							} else {
						...
						case 'B':
							...
							if (args[argn] instanceof java.awt.geom.Rectangle2D) {
								cvt.buildBox(prtbuf, ((java.awt.geom.Rectangle2D) args[argn++]), true,
										flag);
							} else {
								throw new RuntimeException("argument " + argn +
										" should be a Rectangle2D");
						...
						case 'u':
							if ((n + 3) < flen) {
								if (
									...
									int uni = (int) format[n + 3] + 16 * (int) format[n + 2] +
											  256 * (int) format[n + 1] + 4096 * (int) format[n];
						...
						case '9':
							...
							if ((n + 1) < flen) {
								if (
									...
									int oct = (int) format[n + 1] + 8 * (int) format[n] +
											  64 * (int) ch;
...

{
	...
	final StringBuffer
		...
		return (strpad(buf, doubleToString(sign * base, scratch.toString()), padding, width,
				rightpad));
	}
	...
	final StringBuffer
			buildBox(StringBuffer buf, java.awt.geom.Rectangle2D parg, boolean upper,
			boolean dimensioned) {
