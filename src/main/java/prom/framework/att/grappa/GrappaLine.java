 ...
 */
public class GrappaLine implements
		GrappaConstants,
		Cloneable, Shape {
	/**
	 * Arrow head length
    ...
    /**
     * Swap head with tail. Needed when the nexus decides to swap the entire edge.
     */
	...
	// added by little@2007.11.27
	private void addArrow(GeneralPath path, GeneralPath testpath, GrappaPoint tip,
			GrappaPoint shaft, double length, double width, boolean isHead) {
        ...
        if ((!isHead && arrowhead == 1) || (isHead && arrowtail == 1)) {
			...
			x = (float) (tip.x - (half_length * Math.cos(theta) - half_width * Math.sin(theta)));
			y = (float) (tip.y - (half_length * Math.sin(theta) + half_width * Math.cos(theta)));
			path.lineTo(x, y);
			testpath.lineTo(x, y);
			...
			float r = (float) (half_length >= half_width ? half_width : half_length);
			x = (float) (tip.x - (r * Math.cos(theta) + r));
			y = (float) (tip.y - (r * Math.sin(theta) + r));

			java.awt.geom.Ellipse2D.Float circle = new java.awt.geom.Ellipse2D.Float(x, y, 2 * r,
					2 * r);
	...
	private void updateLine(GrappaPoint[] grpts, int type) {
		...
		GeneralPath grtestpath = new GeneralPath(windingRule,
								 grpts.length + xpts + grpts.length - 1);
		...
		if ((type & HEAD_ARROW_EDGE) != 0) {
			grtestpath.moveTo(x + z, y + z);
			addArrow(grpath, grtestpath, grpts[pts - 1], grpts[pts], arrowLength, arrowWidth, true);
		...
		if ((type & TAIL_ARROW_EDGE) != 0) {
			addArrow(grpath, grtestpath, grpts[pts - 1], grpts[pts - 2], arrowLength, arrowWidth, false);
		}
		pts--;
		while (pts > 0) {
