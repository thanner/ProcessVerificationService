...

{
	...
	private static final short ctype[] = {
										 ...
										 DG | OD, DG | OD, DG | OD, DG | OD, DG | OD, DG | OD,
										 DG | OD, DG | OD,
										 DG, DG, PU, PU, PU, PU, PU, PU,
										 PU, UC | HD, UC | HD, UC | HD, UC | HD, UC | HD, UC | HD,
										 UC,
										 UC, UC, UC, UC, UC, UC, UC, UC,
										 UC, UC, UC, UC, UC, UC, UC, UC,
										 UC, UC, UC, PU, PU, PU, PU, PU,
										 PU, LC | HD, LC | HD, LC | HD, LC | HD, LC | HD, LC | HD,
										 LC,
	 ...
	 * @return a GrappaBox generated the possibly-reordered coordinates.
	 */
	public static GrappaBox boxFromCorners(GrappaBox box, double x1, double y1, double x2,
			double y2) {
		if (box == null) {
			box = new GrappaBox();
		}
	...
	public static Element findContainingElement(Subgraph subg, Point2D pt) {
		...
		if (bb.contains(pt)) {
			...
			while (enu.hasMoreElements()) {
				if ((subelem = findContainingElement((Subgraph) (enu.nextElement()), pt)) != null) {
					elem = subelem;
				}
			}
	...
	public static Vector findContainedElements(Subgraph subg, GrappaBox box) {
		...
		if (box.contains(bb)) {
			...
			enu = subg.subgraphElements();
			while (enu.hasMoreElements()) {
				if ((subelems = findContainedElements((Subgraph) (enu.nextElement()), box)) != null) {
					if (elems == null) {
						elems = new Vector();
					}
					elems.add(subelems);
	...
	public static boolean centerPanel(Point2D cpt, GrappaPanel gpanel) {
		...
		if (!(prnt instanceof javax.swing.JViewport)) {
			Grappa.displayException(new RuntimeException(
		...
		Rectangle2D r = new Rectangle2D.Double(p.getX() + (double) b.x - ((double) d.width) / 2.,
						p.getY() + (double) b.y - ((double) d.height) / 2., (double) d.width,
						(double) d.height);
