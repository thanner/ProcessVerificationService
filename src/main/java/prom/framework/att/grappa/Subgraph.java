...
public class Subgraph extends Element implements Comparator {
	...
	public java.awt.geom.Rectangle2D getBoundingBox() {
		...
		if (grappaNexus == null || (bbox = grappaNexus.bbox) == null) {
			...
			if (minSize != null) {
				if (bbox == null) {
					bbox = new java.awt.geom.Rectangle2D.Double(0, 0, minSize.getWidth(),
						   minSize.getHeight());
				} else {
					bbox.add(new java.awt.geom.Rectangle2D.Double(bbox.getCenterX() -
							(minSize.getWidth() / 2.0),
							bbox.getCenterY() - (minSize.getHeight() / 2.0), minSize.getWidth(),
							minSize.getHeight()));
				}
			}
			...
			if (minBox != null) {
				if (bbox == null) {
					bbox = new java.awt.geom.Rectangle2D.Double(minBox.x, minBox.y, minBox.width,
						   minBox.height);
				} else {
					bbox.add(new java.awt.geom.Rectangle2D.Double(minBox.x, minBox.y, minBox.width,
							minBox.height));
				}
			}
			...
			bbox.add(bbox.getX() + bbox.getWidth() + 11, bbox.getY() + bbox.getHeight() + 11);
	...
	public void printSubgraph(PrintWriter out) {
		...
		if (getSubgraph() == null) {
			...
			out.println(indent + (graph.isStrict() ? "strict " : "") + (graph.isDirected() ?
					"digraph" : "graph") + " " + graph.toString() + " {");
	...
	private void printDfltAttr(PrintWriter out, Hashtable dfltAttr, int type, String prefix,
			String suffix) {
		String indent = new String(getGraph().getIndent());
		String value;
		String key;
		...
		while (attrs.hasMoreElements()) {
			...
			if (Grappa.elementPrintAllAttributes || Grappa.elementPrintDefaultAttributes ||
	...
	private Element findElementInSubgraphByName(int type, String name) {
		...
		while (enu.hasMoreElements()) {
			if ((elem = ((Subgraph) enu.nextElement()).findElementInSubgraphByName(type, name)) != null) {
				return elem;
			}
		}
...
/*		if (newNode.getSubgraph() != this) {
			Message.add("A node was added to a graph that was not the same graph as given by the getSubGraph()."+
	...
	*/
			...
			while (enu.hasMoreElements()) {
				...
				if (sg != this) {
					...
					if (attr != null) {
						...
						sg.setAttribute(STYLE_ATTR,
								style != null && style.length() > 0 ? style + ",filled(false)" : null);
					}
				}
			}
		} else {
			...
			while (enu.hasMoreElements()) {
				...
				if (sg != this) {
					sg.setAttribute(COLOR_ATTR,
							java.awt.Color.getHSBColor((float) (0.05 + 0.9 * (nbr++ / sgtot)),
							(float) 1.0, (float) 1.0));
					attr = sg.getAttribute(STYLE_ATTR);
					if (attr == null) {
						...
						style = attr.getStringValue();
						sg.setAttribute(STYLE_ATTR,
								style == null || style.length() == 0 ? "filled" : style + ",filled");
					}
				}
			}
		}
	...
	double score(double wd, double ht) {
		return ((ht <= PATCHEDGE2 || wd <= PATCHEDGE2) ? Double.MAX_VALUE : (ht > wd ? (wd == 0 ?
				(ht == 0 ? 1 : Double.MAX_VALUE) : ht / wd) : (ht == 0 ? (wd == 0 ? 1 :
				Double.MAX_VALUE) : wd / ht)));
	}
	...
	private void compSqPatchWork(java.awt.geom.Rectangle2D.Double r, boolean top) {
		...
		if (top) {
			...
			box = new GrappaBox(r.getX() + PATCHEDGE, r.getY() + PATCHEDGE,
				  r.getWidth() - PATCHEDGE2, r.getHeight() - PATCHEDGE2);
				...
				if ((i + 1) < sgPatches.length) {
					...
					for (; ; ) {
						...
						for (; i < j; i++) {
							el = (Element) sgPatches[i];
							if (dir > 1) {
								p = new GrappaBox(prv, previous,
									nxt = box.getWidth() * el.getPatchSize() / tot, next);
							} else {
					...
					if (dir > 1) {
						...
						p = new GrappaBox(previous, box.getY(), (next = frac * box.getWidth()),
								box.getHeight());
					}
					((Subgraph) el).compSqPatchWork(p, false);
					i++;
				...
				if ((i + 1) < elPatches.length) {
					...
					for (; ; ) {
						...
						for (; i < j; i++) {
							...
							if (el instanceof Node) {
								if (dir > 1) {
									el.setPatch(prv, previous,
											nxt = box.getWidth() * el.getPatchSize() / tot, next);
								} else {
									el.setPatch(previous, prv, next,
											nxt = box.getHeight() * el.getPatchSize() / tot);
								}
								p = el.getPatch();
					...
					if (el instanceof Node) {
						if (dir > 1) {
							...
							el.setPatch(previous, box.getY(),
									(next = frac * box.getWidth()), box.getHeight());
						}
						p = el.getPatch();
	...
	private void compStdPatchWork(java.awt.geom.Rectangle2D.Double r, boolean top) {
		...
		if (sgPatches != null) {
			for (int i = 0; i < sgPatches.length; i++) {
				...
				frac = sz / total;
				if (dir > 1) {
					((Subgraph) el).compStdPatchWork(new GrappaBox(box.getX(), previous,
							box.getWidth(), (next = frac * box.getHeight())), false);
				} else {
	...
	class Enumerator implements GraphEnumeration {
		...
		Enumerator(int t) {
			...
			if (enu.hasMoreElements()) {
				dictType = SUBGRAPH;
				while (enu.hasMoreElements()) {
					subEnum = ((Subgraph) (enu.nextElement())).new Enumerator(types);
			...
			if (enu == null) {
				if ((types & NODE) != 0 && (enu = nodeElements()).hasMoreElements()) {
					dictType = NODE;
					if (elem == null) {
						elem = (Element) enu.nextElement();
					}
				} else if ((types & EDGE) != 0 && (enu = edgeElements()).hasMoreElements()) {
					dictType = EDGE;
					if (elem == null) {
						elem = (Element) enu.nextElement();
					}
