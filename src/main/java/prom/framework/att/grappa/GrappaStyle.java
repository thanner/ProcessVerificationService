 ...
 */
public class GrappaStyle implements
		GrappaConstants,
		Cloneable {
	...
	static BasicStroke defaultStroke = new BasicStroke(STYLE_LINE_WIDTH_DEFAULT, STYLE_CAP_DEFAULT,
									   STYLE_JOIN_DEFAULT, STYLE_MITER_LIMIT_DEFAULT,
									   STYLE_DASH_DEFAULT, STYLE_DASH_PHASE_DEFAULT);

	static String defaultStrokeString = generateStrokeString(STYLE_LINE_WIDTH_DEFAULT,
										STYLE_CAP_DEFAULT, STYLE_JOIN_DEFAULT,
		...
		while (i < len) {
			...
			if (c == '(') {
				...
				if (in_parens) {
					...
					while (i < len && (c = style.charAt(i)) != ',' && c != '(' && c != ')') {
						i++;
					}
				}
				...
				if (in_parens) {
					if (last_option == -1) {
						throw new RuntimeException("style attribute modifier (" + option_string +
								") is unexpected");
					}
					switch (last_option) {
						case STYLE_DASH:
							...
							try {
								...
								throw new RuntimeException("dash style attribute modifier (" +
										option_string +
						...
						case STYLE_DASH_PHASE:
							try {
								...
								throw new RuntimeException("dash phase style attribute modifier (" +
										option_string + ") is not a floating point number");
							}
							break
									;
						case STYLE_LINE_WIDTH:
							try {
								...
								throw new RuntimeException("line width style attribute modifier (" +
										option_string + ") is not a floating point number");
							}
							break
									;
						case STYLE_MITER_LIMIT:
							try {
								...
								throw new RuntimeException("miter limit style attribute modifier (" +
										option_string + ") is not a floating point number");
							}
							break
									;
...
//							font_style = new Integer(Font.ITALIC);
		...
		String strokeString = generateStrokeString(line_width, cap, join, miter_limit, dash,
							  dash_phase);
	...
	public String toAttributeString() {
		return (generateStyleString(line_color, line_style, line_width, cap, join, miter_limit,
				dash, dash_phase, rounded, diagonals, filled, invis, fixed_size, font_style, false,
				elementType));
	}
	...
	public String toString() {
		return (generateStyleString(line_color, line_style, line_width, cap, join, miter_limit,
				dash, dash_phase, rounded, diagonals, filled, invis, fixed_size, font_style, true,
				elementType));
	}
	...
	private static String generateStyleString(
		...
		if (
			...
			if ((tmpstr = GrappaColor.getColorName(color)) == null) {
				...
				styleStringBuffer.append(GrappaSupportPrintf.sprintf(new Object[] {"%g,%g,%g",
						new Float(hsb[0]), new Float(hsb[1]), new Float(hsb[2])}));
			} else {
