 ...
 *
 */
public class CloudChamberPanel extends JComponent implements MouseListener, MouseMotionListener {
	...
	public CloudChamberPanel(CloudChamberStats ccStats) {
		...
		gradientAlpha = 1.0f;
		helpImage = (new ImageIcon(UISettings.getProMDirectoryPath() + "images/cloudchamberdoc.png").getImage());
		gradientHelp = 0.0f;
	}
	...
	protected void paintComponent(Graphics g) {
		...
		g2d.drawImage(renderer.getSection(x1, y1, renderWidth, renderHeight), x1, y1, null);
		...
		for(int i=1; i<instanceBoundaries.length; i++) {
			if(instanceBoundaries[i] >= x1 || instanceBoundaries[i] >= y1) {
				...
				g2d.drawLine(x1, instanceBoundaries[i], x2, instanceBoundaries[i]);
				g2d.drawLine(instanceBoundaries[i], y1, instanceBoundaries[i], y2);
			}
		}
		// draw inspector
		int ptX = mouseX - 10;
		...
		if(gradientAlpha > 0.0f) {
			if(gradientAlpha < 1.0f) {
				AlphaComposite alphaComp = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, gradientAlpha);
		...
		if(gradientHelp != 0.0f) {
			AlphaComposite alphaComp = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, gradientHelp * 0.75f);
			g2d.setComposite(alphaComp);
			g2d.setColor(Color.BLACK);
			g2d.fillRect(x1, y1, renderWidth, renderHeight);
			alphaComp = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, gradientHelp);
			g2d.setComposite(alphaComp);
