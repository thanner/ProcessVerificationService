...
public class RecommendationServiceHandler implements ServiceHandler {
	...
	/* (non-Javadoc)
	 * @see org.processmining.framework.remote.ServiceHandler#handleRequest(java.io.Reader, java.io.Writer)
	 */
	public void handleRequest(BufferedReader in, PrintWriter out) throws IOException {
		...
		while ((received = in.readLine()) != null) {
			...
			if(received.equals(RecommendationQueryProtocol.CLIENT_REQUEST_XML_PROLOG) == true) {
				// handle recommendation request from client
				handleRecommendationRequest(in, out);
				// exit to proceed to goodbye
				break;
			} else if(received.equals(RecommendationQueryProtocol.CLIENT_TRANSMIT_TRACE_PROLOG) == true) {
				// handle recommendation request from client
				handleCompletedExecution(in, out);
				// exit to proceed to goodbye
				break;
			}else if(received.equals(RecommendationQueryProtocol.CLIENT_REQUEST_RESTART_PROLOG) == true) {
				// handle recommendation request from client
				handleRestartRequest(in, out);
				// exit to proceed to goodbye
				break;
			}else if(received.equals(RecommendationQueryProtocol.CLIENT_REQUEST_CLOSE) == true) {
				// handle recommendation request from client
		...
		while ((received = in.readLine()).equals(RecommendationQueryProtocol.CLIENT_GOODBYE) == false) {
	...
	protected void handleRecommendationRequest(BufferedReader in, PrintWriter out) throws IOException {
		String received = null;
		StringBuilder queryXml = new StringBuilder();
		String resultXml = null;
		// receive query XML string
		while ((received = in.readLine()).equals(
						   RecommendationQueryProtocol.CLIENT_REQUEST_XML_EPILOG) == false) {
		...
		try {
			...
			e.printStackTrace();
			Message.add("Exception thrown when unmarshalling your query:"+e.toString() + " ", Message.ERROR);
		...
		try {
			...
			e.printStackTrace();
			Message.add("Exception thrown processing your query:"+e.toString(), Message.ERROR);
		...
		try {
			...
			e.printStackTrace();
			Message.add( "Exception thrown when marshalling the result to your query:"+e.toString(), Message.ERROR);
			sendError(out, "Exception thrown when marshalling the result to your query:", e);
			return; // give up
		}
		// signal success
	...
	protected void handleCompletedExecution(BufferedReader in, PrintWriter out) throws IOException {
		String received = null;
		StringBuilder traceXml = new StringBuilder();
		// receive completed trace XML string
		while ((received = in.readLine()).equals(
						   RecommendationQueryProtocol.CLIENT_TRANSMIT_TRACE_EPILOG) == false) {
			traceXml.append(received);
			traceXml.append('\n'); // add proper newline terminator
		}
		// signal receipt
		writeLine(out, RecommendationQueryProtocol.SERVER_TRANSMIT_TRACE_RECEIVED);
		...
		try {
			...
			sendError(out, "Exception thrown when unmarshalling the completed trace which has been transmitted:", e);
			e.printStackTrace();
		}
	}

	protected void handleRestartRequest(BufferedReader in, PrintWriter out) throws IOException {
		String received = null;
		StringBuilder traceXml = new StringBuilder();
		// receive completed trace XML string
		while ((received = in.readLine()).equals(
						   RecommendationQueryProtocol.CLIENT_REQUEST_RESTART_EPILOG) == false) {
			traceXml.append(received);
			traceXml.append('\n'); // add proper newline terminator
		}
		// signal receipt
		writeLine(out, RecommendationQueryProtocol.SERVER_REQUEST_RESTART_RECEIVED);
		// unmarshal and handle completed trace in
		// local recommendation provider implementation
		try {
			RestartRequest request = restartMarshal.unmarshal(traceXml.toString());
			localProvider.requestRestart(request.getContributor(), request.getScale());
		} catch (Exception e) {
			sendError(out, "Exception thrown when unmarshalling the completed trace which has been transmitted:", e);
			e.printStackTrace();
		}
	...
	protected void handleCloseRequest(BufferedReader in, PrintWriter out) throws IOException {
		String received = null;
		StringBuilder traceXml = new StringBuilder();
		// receive completed trace XML string
		// signal receipt
		writeLine(out, RecommendationQueryProtocol.SERVER_REQUEST_CLOSE_RECEIVED);
		// unmarshal and handle completed trace in
		// local recommendation provider implementation
		try {
	...
	protected void sendError(PrintWriter out, String reason, Exception e) throws IOException {
