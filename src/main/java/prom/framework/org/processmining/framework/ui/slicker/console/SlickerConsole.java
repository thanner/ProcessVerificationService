...
public class SlickerConsole extends JComponent {
	...
	static {
		...
		try {
			...
			addLogLine("ProM execution log for " + dateFormat.format(new Date(System.currentTimeMillis())));
		...
		try {
			...
			addLogLine("ProM test log for " + dateFormat.format(new Date(System.currentTimeMillis())));
			addLogLine("Version: " + About.VERSION + " (" + About.VERSION_DATE + ")");
			addLogLine("----------------------------------------------------------------\n");
		} catch (IOException e) {
			// this is a bad one...
	...
	public void scrollToBottom() {
		this.scrollRectToVisible(new Rectangle(-23, this.getHeight() - 2, 25, 25));
		revalidate();
		repaint();
	}
	...
	protected void drawLine(int index, int x, int y, Graphics2D g2d, boolean shadow) {
		...
		case Message.TEST:
			...
			break;
		default:
			System.err.println("SlickerConsole ERROR:\ncould not map message code " +
	...
	/**
	 * @param showTest the showTest to set
	 */
