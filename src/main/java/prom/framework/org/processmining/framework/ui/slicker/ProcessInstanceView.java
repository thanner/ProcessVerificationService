 ...
 *
 */
public class ProcessInstanceView extends JComponent implements MouseListener, MouseMotionListener {
	...
	public ProcessInstanceView(ProcessInstance instance, LogSummary summary) {
		...
		int width = (instance.getAuditTrailEntryList().size() * elementWidth) + trackPadding + 300;
	...
	protected void paintComponent(Graphics g) {
		...
		int trackRightX = trackPadding + (instance.getAuditTrailEntryList().size() * elementWidth);
		int startX = clip.x - (clip.x % elementWidth);	// shift to left if necessary
		if(startX < trackPadding) { startX = trackPadding; }
		int eventIndex = (startX - trackPadding) / elementWidth;
		for(int x=startX; (x < clipRightX && x < trackRightX); x += elementWidth) {
			this.drawEvent(g2d, eventIndex, (eventIndex == activeEvent), 
					x, trackY, elementWidth, trackHeight);
			eventIndex++;
		}
	...
	protected void drawInstanceFlag(Graphics2D g2d, int x, int y, int height) {
		...
		// draw flag shadow
		int shadowOffset = 4;
		int[] xSCoords = new int[] {x, x + width - elementTriOffset + shadowOffset, x + width + shadowOffset, x + width - elementTriOffset + shadowOffset, x};
		int[] ySCoords = new int[] {y + shadowOffset, y + shadowOffset, y + (height / 2) + shadowOffset, y + height + shadowOffset, y + height + shadowOffset};
	...
	protected void drawEventFlag(Graphics2D g2d, int index, int x, int y, int height) 
		...
		String originator = ate.getOriginator() + "; freq: " + format.format(frequency);
	...
	protected void drawEvent(Graphics2D g2d, int index, boolean active, int x, int y, int width, int height) {
		...
		if(active == true) {
			...
			g2d.drawPolyline(new int[] {x - 3, midPointBX - 3, x - 3}, new int[] {y - 3, midPointY - 3, y + height - 3}, 3);
			g2d.setColor(colorAttenuationBright);
			g2d.drawPolyline(new int[] {x -2, midPointBX -2, x -2}, new int[] {y - 3, midPointY - 3, y + height - 3}, 3);
		} else {
			g2d.setColor(colorAttenuationDark);
