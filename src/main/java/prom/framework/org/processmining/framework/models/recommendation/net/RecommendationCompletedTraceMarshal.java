...
public class RecommendationCompletedTraceMarshal {
	...
	public String marshal(ProcessInstance trace) throws Exception {
		...
		LogSetSequential seqSet = new LogSetSequentialImpl(sPersistency, "ProM recommendation servide", "void");
		// write boilerplate process and process instance header
		seqSet.startProcess("TraceContainer", "Recommendation service created", null);
		seqSet.startProcessInstance(trace.getName(), trace.getDescription(), trace.getAttributes());
		// write events
	...
	public ProcessInstance unmarshal(String traceXml) throws Exception {
		...
		LogReader log = LogReaderFactory.createInstance(new DefaultLogFilter(DefaultLogFilter.INCLUDE), sLogFile);
		return log.getInstance(0);
	}
	
	protected org.processmining.lib.mxml.AuditTrailEntry promAteToMxmlibAte(org.processmining.framework.log.AuditTrailEntry promAte) {
	...
	/**
	 * Internal class to implement a string-based logfile for
	 * the ProM log reader to read from.
	 * 
	 * @author Christian W. Guenther (christian@deckfour.org)
	 *
	 */
