...
public class ShowExposeAction extends CatchOutOfMemoryAction {
	...
	public static Icon getIcon() {
		String customIconPath = UISettings.getInstance().getPreferredIconTheme() + "/toolbar_expose.png";
	...
	public ShowExposeAction(MDIDesktopPane desktop, String label) {
		super(label, desktop);
		putValue(SHORT_DESCRIPTION, "Expos\u00E9 (fast window switching)  " + getShortcut());
	...
	protected void execute(ActionEvent e) {
		if(MainUI.getInstance().getDesktop().getAllFrames().length > 1) {
			...
			MainUI.getInstance().showGlassDialog("Expos\u00E9 not applicable",
