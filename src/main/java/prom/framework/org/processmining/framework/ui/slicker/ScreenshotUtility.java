...
public class ScreenshotUtility {
	...
	public static void captureApplicationScreenshot() {
		...
		FileDialog dialog = new FileDialog(ui, "Save application screenshot...", FileDialog.SAVE);
		...
		if(dialog.getFile() != null) {
			File outFile = new File(dialog.getDirectory() + File.separator + dialog.getFile());
			try {
	...
	public static void captureActiveFrameScreenshot() {
		...
		FileDialog dialog = new FileDialog(MainUI.getInstance(), "Save frame screenshot...", FileDialog.SAVE);
		dialog.setFilenameFilter(new FilenameFilter() {
			public boolean accept(File dir, String name) {
				return (name.endsWith("png") || name.endsWith("PNG"));
			}
