...
public class LaunchActionList {
	...
	protected static final MiningPluginCollection colMining = MiningPluginCollection.getInstance();
	protected static final AnalysisPluginCollection colAnalysis = AnalysisPluginCollection.getInstance();
	protected static final ConvertingPluginCollection colConverting = ConvertingPluginCollection.getInstance();
	protected static final ExportPluginCollection colExport = ExportPluginCollection.getInstance();
	...
	protected void updateData(ProvidedObject object) {
			...
			for(Object obj : object.getObjects()) {
				if(obj instanceof LogReader) {
					for(Plugin miner : colMining.getPlugins()) {
						...
						actions.add(new MineAction((MiningPlugin)miner, object));
						pluginTypes.add(PluginType.MINING);
						names.add(miner.getName().toLowerCase());
					}
				}
				if(isActive == false) { return; }	// abort if indicated
			}
		}
		notifyListeners();
		synchronized(this) {
		...
		synchronized(this) {
			...
			for(int i=0; i<colConverting.size(); i++) {
				ConvertingPlugin algorithm = (ConvertingPlugin)colConverting.get(i);
				if(algorithm == null) { continue; }
