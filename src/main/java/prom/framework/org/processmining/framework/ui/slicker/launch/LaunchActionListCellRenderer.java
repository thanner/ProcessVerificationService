...
public class LaunchActionListCellRenderer extends JLabel implements
	...
	public LaunchActionListCellRenderer(int height) {
		...
		this.bgMining = createBackground(this.colorMining, this.colorMiningHilight);
		this.bgAnalysis = createBackground(this.colorAnalysis, this.colorAnalysisHilight);
		this.bgConverting = createBackground(this.colorConverting, this.colorConvertingHilight);
		this.bgExport = createBackground(this.colorExport, this.colorExportHilight);
		this.bgActive = createBackground(this.colorActive, this.colorActiveHilight);
	}
	
	protected BufferedImage createBackground(Color color, Color hilight) {
		BufferedImage bg = new BufferedImage(1, height, BufferedImage.TYPE_INT_ARGB);
		Graphics2D g2dBuf = (Graphics2D)bg.getGraphics();
		GradientPaint gradient = new GradientPaint(0, 0, color, 0, height / 3, hilight, false);
		g2dBuf.setPaint(gradient);
		g2dBuf.fillRect(0, 0, 1, height / 3);
		gradient = new GradientPaint(0, height / 3, hilight, 0, height, color, false);
		g2dBuf.setPaint(gradient);
	...
	public Component getListCellRendererComponent(JList list, Object value,
		...
		if(value instanceof MineAction) {
			...
			renderer.text = ((ConvertInternalAction)value).getPlugin().getName();
