		...
		try {
			if(processes.contains(instance.getProcess())) {
				...
				if(ateList.size() == 0) {
					...
					LogEvent eFirst = new LogEvent(first.getElement(), first.getType());
					if(startEvents.contains(eFirst) == false) {
						return false;
					}
					// check last event
					AuditTrailEntry last = ateList.get(ateList.size() - 1);
					LogEvent eLast = new LogEvent(last.getElement(), last.getType());
					...
					for(int i=0; i<ateList.size(); i++) {
						...
						if(filterEvents.contains(currentEvent) == false) {
							ateList.remove(i);
							i--;	// correct index (will be incremented in next iteration)
						} else if(removeTypes.contains(currentAte.getType()) == true) {
							ateList.remove(i);
							i--;	// correct index (will be incremented in next iteration)
						} else if(skipInstanceTypes.contains(currentAte.getType()) == true) {
							// instance is skipped
							return false;
						}
