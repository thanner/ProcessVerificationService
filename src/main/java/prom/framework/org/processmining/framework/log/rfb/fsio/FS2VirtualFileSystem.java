...
public class FS2VirtualFileSystem implements StorageProvider {
	...
	/*
	 * This installs the shutdown hook, which will clear all files
	 * backing swap data from this class when the virtual machine
	 * shuts down. The shutdown hook is installed at the moment, at
	 * which this class is first loaded by the JVM.
	 */
	static {
		Runtime.getRuntime().addShutdownHook(new ShutdownHook());
		Message.add("NikeFS2: Shutdown hook installed for terminal cleanup tasks.", Message.DEBUG);
	}
	 ...
	 */
	public static final File SWAP_DIR = new File(System.getProperty("java.io.tmpdir"));
	...
	public synchronized FS2Block allocateBlock() 
		...
		Message.add("NikeFS2: Allocating new swap file. (#" + (blockProviders.size() + 1) + ": " + swapFileSize + " bytes)", Message.DEBUG);
		File swapFile = File.createTempFile(
				FS2VirtualFileSystem.SWAP_PREFIX, 
				FS2VirtualFileSystem.SWAP_SUFFIX, 
				FS2VirtualFileSystem.SWAP_DIR );
		FS2BlockProvider addedProvider = new FS2BlockProvider(swapFile, swapFileSize, blockSize, true);
	...
	protected static class ShutdownHook extends Thread {
		 ...
		 * VirtualFileSystem from the file system.
		 */
		public void run() {
			...
			for(int i=0; i<tmpFiles.length; i++) {
				if(tmpFiles[i].getName().matches(tmpFileRegEx)) {
					if(tmpFiles[i].delete()==false) { tmpFiles[i].deleteOnExit(); }
					cleaned++;
				}
			}
