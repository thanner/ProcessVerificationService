...
public class PetriNet extends ModelGraph implements Cloneable {
     ...
     *
     * @param tc the TransitionCluster to be added
     */
     ...
     *
     * @param p1 the first place to be merged
     * @param p2 the second place to be merged
    ...
    /**
     * @param e the LogEvent of the transition to be added
     * @return the newly added transition
     ...
     *
     * @param identifier the string specifying the identifier
     ...
     * Adds ...
     *
     * @param pn the Petri net to be added
     ...
     *
     * @param p the place specified as the source node for the new edge
     * @param t the transition specified as the target node for the new edge
     ...
     *
     * @param e the edge to be added from p to t
     * @param p the place specified as the source node for the new edge
     * @param t the transition specified as the target node for the new edge
     * @return <code>true</code> if both the transition and the place are
     * contained in this Petri net, <code>false</code> otherwise
     ...
     * (target node).
     *
     * @param t the transition specified as the source node for the new edge
     * @param p the place specified as the target node for the new edge
     * @return <code>true</code> if both the transition and the place are
     * contained in this Petri net, <code>false</code> otherwise
     */
     ...
     * (target node).
     *
     * @param e the edge to be added from t to p
     * @param t the transition specified as the source node for the new edge
     * @param p the place specified as the target node for the new edge
     * @return <code>true</code> if both the transition and the place are
     * contained in this Petri net, <code>false</code> otherwise
     ...
     *
     * @param bw the writer used by the framework to create the temporary dot
     *           file
    ...
    protected void writeClustersToDot(Writer bw) throws IOException {
        ...
        for (i = 0; i < transitionClusters.size(); i++) {
            TransitionCluster trans = transitionClusters
                    .get(i);
     ...
     *
     * @param timestamp the date to be attached with the token
     * @param p         the place where the token should be added
     ...
     *
     * @param lme the log event specified
    ...
    public Transition findRandomTransition(LogEvent lme) {
        if (findTransitions(lme).size() == 0) {
            ...
            return findTransitions(lme).get(0);
        }
    }
    ...
    /**
     * @param e the LogEvent to compare with
     ...
     *
     * @param t the transition to compare with
     * @return an equal transition, if existent. <code>Null</code> otherwise.
     */
     ...
     *
     * @param transitions the transition cluster for which the contained places should
     *                    be found
     ...
     * successor nodes.
     *
     * @param in  the set of predecessor nodes
     * @param out the set of successor nodes
     ...
     * assumed to be unique).
     *
     * @param head the node specifying the head of the edge
     * @param tail the node specifying the tail of the edge
    ...
    public void makeClusters() {
        ...
        for (int i = 0; i < nme; i++) {
            if ((clustered.get(i) == 1)
                    || (transitions.get(i).getLogEvent() == null)) {
                continue;
            }
            IntArrayList trans = new IntArrayList();
            for (int j = i; j < nme; j++) {
                if (transitions.get(j).getLogEvent() == null) {
                    continue;
                }
                if (!transitions.get(i).getLogEvent()
                        .getModelElementName().equals(
                                transitions.get(j).getLogEvent()
                                        .getModelElementName())) {
                    continue;
                }
                clustered.set(j, 1);
            ...
            if (trans.size() >= 2) {
                TransitionCluster tc = new TransitionCluster(
                        transitions.get(trans.get(0))
                                .getLogEvent().getModelElementName());
                for (int j = 0; j < trans.size(); j++) {
                    tc
                            .addTransition(transitions.get(trans
                                    .get(j)));
                }
                addCluster(tc);
            }
    ...
    public boolean hasDuplicatesWithSharedInputPlaces() {
        ...
        duplSearch:
        // label to abort outmost loop as soon as one shared input
        // was found
        while (allTransitions.hasNext()) {
            Transition trans = allTransitions.next();
     ...
     *
     * @param A set of nodes that belongs to this PetriNet
        ...
        while (edges.hasNext()) {
            ...
            if (nodeNames.contains(edge.getHead().getName())
                ...
                if (edge.isPT()) {
                    ...
                    // assumed to
                    // be unique)
                    Place myPlace = o.findPlace(p.getIdentifier());
                    Transition t = (Transition) edge.getDest();
                    // find respective transition in this net
                    Transition myTransition = mapping.get(t);
    ...
    /**
     * @see ModelGraph#removeVertex(ModelGraphVertex)
     */
