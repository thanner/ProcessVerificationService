...
public class OrgModel implements Cloneable {
    ...
    public boolean changeOrgEntityID(String old_id, String new_id) {
        ...
        if (orgEntities.containsKey(old_id)) {
            if (!orgEntities.containsKey(new_id)) {
                OrgEntity tempEntity = orgEntities.get(old_id);
    ...
    public boolean changeResourceID(String old_id, String new_id) {
        ...
        if (resources.containsKey(old_id)) {
            if (!resources.containsKey(new_id)) {
                Resource tempRes = resources.get(old_id);
    ...
    public boolean changeTaskID(String old_id, String new_id) {
        ...
        if (tasks.containsKey(old_id)) {
            if (!tasks.containsKey(new_id)) {
                Task tempTask = tasks.get(old_id);
    ...
    public void removeOrgEntity(OrgEntity orgEntity) {
        ...
        String[] key = resources.keySet().toArray(new String[resources.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            Resource tempResource = resources.get(key[i]);
            tempResource.removeOrgEntity(orgEntity);
        }

        key = tasks.keySet().toArray(new String[tasks.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            Task tempTask = tasks.get(key[i]);
    ...
    public Task getTask(LogEvent le) {
        String[] key = tasks.keySet().toArray(new String[tasks.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            Task tempTask = tasks.get(key[i]);
    ...
    public ArrayList<String> getOrgEntityStringList() {
        ...
        String[] key = orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
    ...
    public ArrayList<OrgEntity> getOrgEntityList() {
        ...
        for (int i = 0; i < key.length; i++) {
            list.add(orgEntities.get(key[i]));
        }
    ...
    public List<String> getOrgEntityList(Resource res) {
        ...
        while (iterator.hasNext()) {
            ...
            OrgEntity orgunit = orgEntities.get(key);
    ...
    public ArrayList<String> getOrgEntityStringList(String type) {
        ...
        for (int i = 0; i < key.length; i++) {
            OrgEntity temp = orgEntities.get(key[i]);
    ...
    public ArrayList<OrgEntity> getOrgEntityList(String type) {
        ...
        for (int i = 0; i < key.length; i++) {
            OrgEntity orgentity = orgEntities.get(key[i]);
    ...
    public List<String> getOrgEntityList(Resource res, String type) {
        ...
        while (iterator.hasNext()) {
            String key = iterator.next();
            OrgEntity orgentity = orgEntities.get(key);
    ...
    public ArrayList<String> getResourceListInOrgEntity(OrgEntity orgEntity) {
        ...
        for (int i = 0; i < key.length; i++) {
            if (resources.get(key[i]).hasOrgEntity(orgEntity))
                list.add(key[i]);
        }
        Collections.sort(list);
        return list;
    ...
    public void reallocateOrgModel(ArrayList<ArrayList<String>> listGroups, OrgModel originalModel) {
        ...
        String[] key_resource = resources.keySet().toArray(new String[resources.keySet().size()]);
        for (int i = 0; i < key_resource.length; i++) {
            Resource tempRes = resources.get(key_resource[i]);
            String[] key_orgEntity = orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
            for (int j = 0; j < key_orgEntity.length; j++) {
                OrgEntity tempEntity = orgEntities.get(key_orgEntity[j]);
        ...
        String[] key_task = tasks.keySet().toArray(new String[tasks.keySet().size()]);
        for (int i = 0; i < key_task.length; i++) {
            Task tempTask = tasks.get(key_task[i]);
            String[] key_orgEntity = orgEntities.keySet().toArray(new String[orgEntities.
                    keySet().size()]);
            for (int j = 0; j < key_orgEntity.length; j++) {
                OrgEntity tempEntity = orgEntities.get(key_orgEntity[j]);
        ...
        for (int i = 0; i < listGroups.size(); i++) {
            ArrayList<String> group = listGroups.get(i);
            ...
            for (int j = 0; j < group.size(); j++) {
                Resource res = getResource(group.get(j));
                res.addOrgEntity(tempEntity);
            }
        }

        String[] originalTaskKey = originalTasks.keySet().toArray(new String[originalTasks.keySet().size()]);
        for (int i = 0; i < originalTaskKey.length; i++) {
            Task existingTask = tasks.get(originalTaskKey[i]);
            Task originalTask = originalTasks.get(originalTaskKey[i]);

            HashSet<OrgEntity> originalOrgEntities = originalTask.getOrgEntities();
            String[] originalResourceKey = originalResources.keySet().toArray(new String[originalResources.keySet().size()]);
            ...
            for (int j = 0; j < originalResourceKey.length; j++) {
                ...
                while (it.hasNext()) {
                    OrgEntity originalOrgEntity = it.next();
            ...
            for (int j = 0; j < originalResourceList.size(); j++) {
                String[] orgEntitiesKey = orgEntities.keySet().toArray(new String[
                        orgEntities.keySet().size()]);
                for (int k = 0; k < orgEntitiesKey.length; k++) {
                    OrgEntity existingOrgEntity = orgEntities.get(orgEntitiesKey[k]);
    ...
    public void writeToXML(Writer bw) throws IOException {
        ...
        String[] key = orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            orgEntities.get(key[i]).writeToXML(bw);
        }

        key = resources.keySet().toArray(new String[resources.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            resources.get(key[i]).writeToXML(bw);
        }

        //write hierarchy relationship using implication
        key = orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            orgEntities.get(key[i]).writeToXML2(bw);
        }

        key = tasks.keySet().toArray(new String[tasks.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
            tasks.get(key[i]).writeToXML(bw);
        }
        bw.write("</OrgModel>");
    }
    ...
    public void writeToDot(Writer bw) throws IOException {
        ...
        key = orgEntities.keySet().toArray(new String[orgEntities.keySet().
                size()]);
        for (String aKey : key) {
            nodelist.put(orgEntities.get(aKey), String.valueOf("node" + nodenum));
            bw.write("node" +
                    nodenum++ +
                    " [shape=\"house\",style=\"filled\",fillcolor=\"mediumturquoise\",label=\"" +
                    orgEntities.get(aKey).getName() + "\"];\n");
        }
        // for hierarchy among OrgEntity
        for (String aKey : key) {
            OrgEntity orgEntity = orgEntities.get(aKey);
            for (String bKey : orgEntity.getSubEntityIDs()) {
                OrgEntity orgEntity2 = orgEntities.get(bKey);
                bw.write(nodelist.get(orgEntity2) + " -> " +
        ...
        key = resources.keySet().toArray(new String[resources.keySet().size()]);
        for (String aKey : key) {
            nodelist.put(resources.get(aKey), String.valueOf("node" + nodenum));
            bw.write("node" +
                    nodenum++ +
                    " [shape=\"ellipse\",style=\"filled\",fillcolor=\"lightpink\",label=\"" +
                    resources.get(aKey).getName() + "\"];\n");
        }
        // write tasks
        key = tasks.keySet().toArray(new String[tasks.keySet().size()]);
        for (int i = 0; i < key.length; i++) {
        ...
        for (int i = 0; i < key.length; i++) {
            ...
            String[] key2 = orgEntities.keySet().toArray(new String[orgEntities.keySet().
                    size()]);
            for (int j = 0; j < key2.length; j++) {
                OrgEntity orgEntity = orgEntities.get(key2[j]);

                if (res.hasOrgEntity(orgEntity))
                    bw.write(nodelist.get(res) + " -> " + nodelist.get(orgEntity) +
                            " [label=\" \"];");
            }
        }
        ...
        for (int i = 0; i < key.length; i++) {
            ...
            for (int j = 0; j < key2.length; j++) {
                ...
                if (task.hasOrgEntity(orgEntity))
                    bw.write(nodelist.get(orgEntity) + " -> " + nodelist.get(task) +
                            " [label=\" \"];");
            }
        }
        bw.write("}\n");
    ...
    public void writeToDot(Writer bw, boolean bOrgEntity, boolean bResource, boolean bTask) throws IOException {
        ...
        if (bTask) {
            key = tasks.keySet().toArray(new String[tasks.keySet().size()]);
            for (String aKey : key) {
                nodelist.put(tasks.get(aKey), String.valueOf("node" + nodenum));
                bw.write("node" +
                        nodenum++ + " [shape=\"box\",style=\"filled\",fillcolor=\"wheat\",label=\"" +
                        tasks.get(aKey).getName() + "\\n" +
                        tasks.get(aKey).getEventType() + "\"];\n");
            }
        }
        if (bResource && bOrgEntity) {
            key = resources.keySet().toArray(new String[resources.keySet().size()]);
            for (String aKey : key) {
                Resource res = resources.get(aKey);

                String[] key2 = orgEntities.keySet().toArray(new String[orgEntities.
                        keySet().
                        size()]);
                for (String bKey : key2) {
                    OrgEntity orgEntity = orgEntities.get(bKey);
        ...
        if (bTask && bOrgEntity) {
            ...
            for (String aKey : key) {
                String[] key2 = orgEntities.keySet().toArray(new String[orgEntities.
                        keySet().
                        size()]);
                Task task = tasks.get(aKey);
                for (String bKey : key2) {
                    OrgEntity orgEntity = orgEntities.get(bKey);

                    if (task.hasOrgEntity(orgEntity))
                        bw.write(nodelist.get(task) + " -> " +
                                nodelist.get(orgEntity) +
    ...
    public Object clone() {
        ...
        for (int i = 0; i < key.length; i++) {
            Resource res = resources.get(key[i]);
            Resource clonedRes = o.getResource(key[i]);

            String[] key2 = orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
            for (int j = 0; j < key2.length; j++) {
                OrgEntity orgEntity = orgEntities.get(key2[j]);
                if (res.hasOrgEntity(orgEntity)) {
                    OrgEntity clonedOrgEntity = o.orgEntities.get(key2[j]);
        ...
        for (int i = 0; i < key.length; i++) {
            String[] key2 = orgEntities.keySet().toArray(new String[orgEntities.keySet().size()]);
            Task task = tasks.get(key[i]);
            Task clonedTask = o.getTask(key[i]);
