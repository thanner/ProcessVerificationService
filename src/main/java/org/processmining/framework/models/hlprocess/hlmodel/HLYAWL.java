...
/**
 * High-level process model implementation for YAWL models.
 * <br>
...
public class HLYAWL extends HLModel {
     ...
     *
     * @param aYawlModel the underlying (low-level) YAWL model
     */
    ...
    protected void initialize() {
        ...
        for (YAWLDecomposition decomposition : yawlModel.getDecompositions()) {
            if (decomposition.isRoot() == true) {
                // TODO Anne: check how to handle hierarchical models. For now only handle the nodes from the
                // the root net
    ...
    private void initActivities(YAWLDecomposition decomposition) {
        for (YAWLNode node : decomposition.getNodes()) {
            ...
            if (node instanceof YAWLTask) {
                HLActivity hlActivity = new HLActivity(node.getIdentifier(), hlProcess);
    ...
    private void initChoices(YAWLDecomposition decomposition) {
        // nodes need to be in place when creating the choices as they are condition targes
        for (YAWLNode node : decomposition.getNodes()) {
            ...
            if ((node instanceof YAWLTask && ((YAWLTask) node).getSplitType() == EPCConnector.XOR)
                ...
                HLChoice hlChoice = new HLChoice(node.getIdentifier(), hlProcess);
                ...
                for (Edge edge : node.getOutEdges()) {
                    ...
                    // in case of condition being the target, add all following tasks as condition targets
    ...
    public List<ModelGraphVertex> getGraphNodes() {
        ...
        for (YAWLDecomposition decomposition : ((YAWLModel) model).getDecompositions()) {
