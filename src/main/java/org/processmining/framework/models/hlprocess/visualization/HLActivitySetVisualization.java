...
public class HLActivitySetVisualization extends ActivityGraph {
    ...
    public HLActivitySetVisualization(HLActivitySet actSet, Set<Perspective> thePerspectivesToShow) {
        super();
        ...
        Iterator<ModelGraphVertex> vertices = graph.getActivityVertices().iterator();
...
//		ArrayList<ModelGraphVertex> nodeList =  new ArrayList<ModelGraphVertex>();
//		Subgraph graph2 = graph.getGrappaVisualization().getSubgraph().getGraph();
...
//		}
//
//		//Iterator<ModelGraphVertex> vertices = graph.getVerticeList().iterator();
...
//	}
    ...
    public void writeToDot(Writer bw) throws IOException {
        ...
        if (perspectivesToShow.contains(Perspective.DATA_AT_TASKS)) {
            // visualize the data attributes
            Iterator<HLAttribute> dataAttrs = hlActivitySet.getHLProcess().getAttributes().iterator();
        ...
        if (perspectivesToShow.contains(Perspective.ROLES_AT_TASKS)) {
            // visualize the groups
            Iterator<HLGroup> groups = hlActivitySet.getHLProcess().getGroups().iterator();
            int counterGroups = 0;
            ...
            Iterator<HLResource> resources = hlActivitySet.getHLProcess().getResources().iterator();
        ...
        for (int i = 0; i < activities.size(); i++) {
            ModelGraphVertex av = activities.get(i);
            ...
            if (perspectivesToShow.contains(Perspective.TIMING_EXECTIME)) {
                act.getExecutionTime().writeDistributionToDot("i" + i, "node" + av.getId(), "Execution time:", bw);
            }
            if (perspectivesToShow.contains(Perspective.TIMING_WAITTIME)) {
                act.getWaitingTime().writeDistributionToDot("i" + i, "node" + av.getId(), "Waiting time:", bw);
            }
            if (perspectivesToShow.contains(Perspective.TIMING_SOJTIME)) {
                act.getSojournTime().writeDistributionToDot("i" + i, "node" + av.getId(), "Sojourn time:", bw);
            }
            ...
            if (perspectivesToShow.contains(Perspective.DATA_AT_TASKS)) {
                ...
                while (inputDataAttrs.hasNext()) {
                    ...
                    // write the edge
                    bw.write("node" + av.getId() + " -> " + boxID + " [dir=none, style=dotted];\n");
                }
