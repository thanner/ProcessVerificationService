...
/**
 * Container class for enabling conditions in the context of
 * a high level choice.
 * <br>
 * Will be related to some high level activity in the process
 * and describes pre-conditions that need to be fulfilled
 * to activate the activity (e.g., based on data or probabilistic
 * values).
 *
 * @author Anne Rozinat
 */
public class HLCondition implements Cloneable {
    ...
    public HLActivity getTarget() {
        // TODO Anne: check whether ID and retrieval via HLProcess needed instead
     ...
     *
     * @return the data dependency that must be fulfilled to enable the associated
     * activity
     */
     ...
     *
     * @param expression the new data dependency that must be fulfilled to enable
    ...
    /**
     * Retrieves the relative frequency to enable the activity (to be seen in the context of
     * other enabling targets of the choice).
     *
    ...
    /**
     * Specifies a new relative frequency to enable the activity (to be seen in the context of
     * other enabling targets of the choice).
     *
     * @param frequency the new relative frequency to enable the associated activity
     */
     ...
     *
     * @param probability the new probability to enable the associated activity
     */
...
//			return 0;
//		} else if (((HLCondition) o).getExpression() != null && getExpression() != null &&
//				((HLCondition) o).getExpression().getOrderRank() != -1 && getExpression().getOrderRank() != -1 &&
//				((HLCondition) o).getExpression().getOrderRank() > getExpression().getOrderRank()) {
//			// order rank must be provided for both expressions and the provided one needs to be higher 
//			// than this one
//			return 1;
//		} else {
//			return 0;
