...
public class HLExpressionManager {
     ...
     *
     * @param toBeNegated root element of the expression to be negated
     * @return the root element of the negated expression
     */
    public static HLExpressionOperator negateExpression(HLDataExpression toBeNegated) {
        ...
        notOp.addSubExpression(clonedExpr.getRootExpressionElement().getExpressionNode());
        return notOp;
    }

    /**
     * Encapsulates {@link #connectExpressionsWithOr(List)} to take a list of conditions
     * of which the expressions should be connected.
     */
    public static HLExpressionElement connectConditionsWithOr(List<HLCondition> conditions) {
    ...
    /**
     * Connects the expressions of the given list of conditions with an OR operator.
     *
     * @param conditions the conditions of which the expressions should be connected
     * @return the root element of the OR-connected expression
     */
    public static HLExpressionElement connectExpressionsWithOr(List<HLDataExpression> expressions) {
        if (expressions.size() > 1) {
            ...
            for (HLDataExpression expr : expressions) {
                ...
                if (otherExpr != null) {
                    HLDataExpression clonedExpr = (HLDataExpression) expr.clone();
                    orOp.addSubExpression(clonedExpr.getRootExpressionElement().getExpressionNode());
                }
            }
            return orOp;
        } else {
            HLDataExpression clonedExpr = (HLDataExpression) expressions.get(0).clone();
