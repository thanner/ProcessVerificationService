...
/**
 * Holds information about global characteristics of the process.
 * For example, the case arrival rate that was measured, or specified,
 * for the corresponding high level process.
 *
 * @see HLProcess
 */
public class HLGlobal implements Cloneable {
     ...
     * Default constructor.
     *
     * @param theProcess the high-level process this global info refers to
     */
     ...
     *
     * @param name the name of the high level process
     */
    ...
    /**
     * Adds a new perspective, i.e., indicates that information was provided for this
     * perspective.
     *
     * @param perspective the new perspective that is covered by the high level process
     */
    ...
    /**
     * Retrieves the set of perspectives that are covered by this high level process.
     *
     ...
     *
     * @return the case arrival distribution of the process.
     * If no case generation scheme has been set, a default distribution
     * will be returned.
     */
     ...
     *
     * @param dist the distribution for the case arrival scheme of the process
     */
     ...
     * <br>
     * Note that currently it is only possible to specify the time unit globally.
     * That is, it will apply to all time-related values in the high level process.
     *
     ...
     * That is, it will apply to all time-related values in the high level process.
     *
     * @param unit the time unit to be used for interpretation
     * @see #getCaseGenerationScheme(HLDistribution)
     * @see HLActivity#getExecutionTime()
     * @see HLActivity#getWaitingTime()
     * @see HLActivity#getSojournTime()
     ...
     * <p>This methods changes current values as they are recalculated based on the
     * previous and the new time unit. Note that this can lead to rounding problems
     * as in CPN Tools the time is in integers only.
     * <p>If simply the reference time should be set,
     * then use the method {@link #setTimeUnit(TimeUnit)}} instead.
     *
     * @param unit the new time unit to be used
     * @see #getCaseGenerationScheme(HLDistribution)
     * @see HLActivity#getExecutionTime()
     * @see HLActivity#getWaitingTime()
     * @see HLActivity#getSojournTime()
    ...
    public void changeTimeUnit(TimeUnit unit) {
        if (getTimeUnit() != unit) {
            ...
            double conversionValue = (double) getTimeUnit().getConversionValue() / (double) unit.getConversionValue();
            // apply the conversion to each highleveltransition
            for (HLActivity hlTransition : process.getActivities()) {
                hlTransition.getExecutionTime().setTimeMultiplicationValue(conversionValue);
                hlTransition.getSojournTime().setTimeMultiplicationValue(conversionValue);
                hlTransition.getWaitingTime().setTimeMultiplicationValue(conversionValue);
            }
            // apply the conversion to the case generation scheme
            getCaseGenerationScheme().setTimeMultiplicationValue(conversionValue);
    ...
    public Object clone() {
        ...
        try {
            ...
            o.caseGenerationScheme = (HLDistribution) this.caseGenerationScheme.clone();
            // cloned the coveredperspectives
            o.coveredPerspectives = (Set<Perspective>) ((HashSet<Perspective>) this.coveredPerspectives).clone();
            return o;
    ...
    public void writeToTestLog() {
        ...
        // timing info
        Message.add("DistributionType of case generation scheme: " + this.getCaseGenerationScheme().getDistributionType(), Message.TEST);
        // number of covered perspectives
        Message.add("Number of Covered Perspectives [" + this.getPerspectives().size() + "]", Message.TEST);
        Message.add("</HLGlobal>", Message.TEST);
    }
}
