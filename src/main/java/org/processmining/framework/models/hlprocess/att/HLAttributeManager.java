...
public class HLAttributeManager {
    ...
    /**
     * Creates a new attribute of the requested type based on the passed old attribute
     * parameters, as far as they are applicable for the new type.
     *
     * @param type         the type of attribute that should be created
     * @param oldAttribute the attribute in its previous type
     * @return the new attribute of the requested type
     */
    public static HLAttribute switchAttributeType(AttributeType type, HLAttribute hlAttribute) {
        HLID oldID = hlAttribute.getID();
        if (type == AttributeType.Boolean) {
            hlAttribute = new HLBooleanAttribute(hlAttribute.getName(), hlAttribute.getHLProcess());
        } else if (type == AttributeType.Numeric) {
            hlAttribute = new HLNumericAttribute(hlAttribute.getName(), hlAttribute.getHLProcess());
        } else if (type == AttributeType.Nominal) {
            hlAttribute = new HLNominalAttribute(hlAttribute.getName(), hlAttribute.getHLProcess());
        } else {
     ...
     *
     * @param att the nominal attribute for which the type is checked
    ...
    public static HLAttribute autoChangeType(HLNominalAttribute att) {
        ...
        if (att.getPossibleValues().getValues().size() == 2 ||
            ...
            for (String val : att.getPossibleValues().getValues()) {
                ...
                if (val.equals("yes") || val.equals("true") || val.equals("0") ||
                    ...
                    trueFreq = att.getPossibleValues().getFrequencyPossibleValueNominal(val);
                } else if (val.equals("no") || val.equals("false") || val.equals("1") ||
                        val.equals("neg") || val.equals("negative")) {
                    falseFreq = att.getPossibleValues().getFrequencyPossibleValueNominal(val);
                } else {
                    isBoolean = false;
                    continue;
                }
        ...
        for (String val : att.getPossibleValues().getValues()) {
            ...
            try {
                ...
                int freq = att.getPossibleValues().getFrequencyPossibleValueNominal(val);
        ...
        HLGeneralDistribution dist = new HLGeneralDistribution(mean, mean, var, min, max);
    ...
    /**
     * Checks whether the given nominal attribute can be converted
     * into a numeric attribute.
     *
     * @param att the numeric attribute which should be checked
