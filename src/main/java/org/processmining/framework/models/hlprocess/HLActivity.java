...
/**
 * Holds information with respect to an activity in the process.
 * <br>
 * Note that an activity is defined independently of any concrete task node in the
 * context of a process model (e.g., a function in an EPC model).
 * <br>
 * If the activity is to be related to a specific node in a process model, then this
 * can be done in the high level process implementation for the corresponding
 * process model type (e.g., a Place for the HLPetriNet).
 *
...
//	/** the data attributes that are available for this transition and their modification type*/
//	private HashMap<HLID, TransformationType> dataAttributes = new HashMap<HLID, TransformationType>();
    /**
     * Added by Mariska Netjes, the input data attributes that are available for this transition and their modification type
     */
    private HashMap<HLID, TransformationType> inputDataAttributes = new HashMap<HLID, TransformationType>();
    /**
     * Added by Mariska Netjes, the output data attributes that are available for this transition and their modification type
     * In case there is no distinction between input and output, all data attributes are output data attributes
     */
    private HashMap<HLID, TransformationType> outputDataAttributes = new HashMap<HLID, TransformationType>();

    /**
     * Default constructor (automatically registers the new activity for
     * the given high level process).
     *
     * @param name  the name of this high-level activity
    ...
    /**
     * Adds a new data attribute reference to this activity (with default transformation type "Resampled").
     * <br>
     * Note that if this data attribute had already been added to this activity before,
     * the method has no effect. It is assumed that the attribute is already part of the high level process.
     * @param attID the ID of the attribute to be added
     */
...
//        }
    ...
    /**
     * Adds a new input data attribute reference to this activity (with default transformation type "Resampled").
     * <br>
     * Note that if this input data attribute had already been added to this activity before,
     * the method has no effect. It is assumed that the attribute is already part of the high level process.
     *
     * @param attID the ID of the attribute to be added
     */
    ...
    /**
     * Adds a new output data attribute reference to this activity (with default transformation type "Resampled").
     * In case there is no distinction between input and output data attributes, a data attribute has to be added
     * to the outputDataAttributes.
     * <br>
     * Note that if this output data attribute had already been added to this activity before,
     * the method has no effect. It is assumed that the attribute is already part of the high level process.
     *
     * @param attID the ID of the attribute to be added
     */
...
//	}
    ...
    /**
     * Checks whether the given input data attribute is used for this
     * activity.
     *
     * @param attID the ID of the attribute for which we want to check
     * @return <code>true</code> if the data attribute is referenced,
    ...
    public boolean hasInputDataAttribute(HLID attID) {
        return inputDataAttributes.keySet().contains(attID) == true;
    }

    /**
     * Checks whether the given output data attribute is used for this
     * activity.
     *
     * @param attID the ID of the attribute for which we want to check
     * @return <code>true</code> if the data attribute is referenced,
    ...
    public boolean hasOutputDataAttribute(HLID attID) {
        return outputDataAttributes.keySet().contains(attID) == true;
    }
...
//	}
     ...
     * provided by this activity. If no input data attributes have been set an empty list
     * will be returned
     */
     ...
     *
     * @return the list of HLAttribute objects representing the output attributes
     * provided by this activity. If no output data attributes have been set an empty list
     * will be returned
     */
...
//	}
    ...
    /**
     * Specifies a set of input data attributes for this activity.
     * <br>
     *
     * @param map the map with the ID of the attribute and its transformation type
     */
    ...
    /**
     * Specifies a set of output data attributes for this activity.
     * <br>
     *
     * @param map the map with the ID of the attribute and its transformation type
     */
     ...
     *
     * @param attID the ID of the attribute for which the transformatin type is requested
     * @return the transformation type of the requested attribute.
     * Can be <code>null</code> if attribute not provided by this activity
     */
    ...
    /**
     * Specifies the transformation type for the given data attribute.
     * <br>
     * Adds the referenced attribute if not yet contained.
     *
     * @param attID the ID of the data attribute for which the new modification type is provided
     * @param type  the new transformation type of this data attribute
     */
     ...
     * its completion.
     *
     * @return the distribution of the execution time. In the case that no execution time
     * has been set a default distribution will be returned
     * @see #getWaitingTime()
     * @see #getSojournTime()
     ...
     * its completion.
     *
     * @param dist the new distribution of the execution time
     ...
     *
     * @return the distribution of the waiting time. In the case that no waiting time
     * has been set a default distribution will be returned
     ...
     * enabled (that is, ready to be started) until its actual start.
     *
     * @param dist the new distribution of the waiting time
     ...
     *
     * @return the distribution of the sojourn time. In the case that no sojourn time
     * has been set a default distribution will be returned
     * @see #getExecutionTime()
     * @see #getWaitingTime()
     * @see HLGlobal#getTimeUnit()
     ...
     * The sojourn time is defined as the waiting time + the execution time.
     *
     * @param dist the new distribution of the sojourn time
     * @see #setExecutionTime(HLDistribution)
     * @see #setWaitingTime(HLDistribution)
     * @see HLGlobal#setTimeUnit()
     */
     ...
     * <br>
     * Note that it is currently not possible to assign multiple groups to an activity.
     *
     * @return the group of resources that may execute this activity if specified.
    ...
    /**
     * Specifies a group for this activity.
     * <br>
     * Note that it is currently not possible to assign multiple groups to an activity.
     *
     * @param groupID the ID of the group of resources that may execute this activity
     */
    ...
    /**
     * Removes the specified attribute from this activity.
     * <br>
...
//        }
    ...
    /**
     * Removes the specified input attribute from this activity.
     * <br>
     * Does nothing if not found.
     *
     * @param id the id of the attribute to be removed
    ...
    /**
     * Removes the specified output attribute from this activity.
     * <br>
     * Does nothing if not found.
     *
     * @param id the id of the attribute to be removed
     ...
     * If the given group was not assigned to this activity in the first place,
    ...
    /**
     * Indicates whether this activity is automatic. An activity is automatic when the nobody
     * group has been attached to it.
     *
    ...
    public Object clone() {
        ...
        try {
            ...
            o.inputDataAttributes = (HashMap<HLID, TransformationType>) inputDataAttributes.clone();
            //clone the output data attributes
            o.outputDataAttributes = (HashMap<HLID, TransformationType>) outputDataAttributes.clone();
            return o;
