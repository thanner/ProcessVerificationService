...
/**
 * Class defining static enums that represent simple data types used in the context
 * of the high level processes in ProM.
 */
public class HLTypes {
    ...
    public enum TimeUnit {
        ...
        MINUTES("Minutes", 60),
        SECONDS("Seconds", 1),
        MILLISECONDS("Milliseconds", 1); // milliseconds not supported by cpn export
         ...
         */
        TimeUnit(String name, int conversionValue) {
            myName = name;
    ...
    /**
     * Class representing a perspective that may be covered by the HighLevelProcess
     * A HighLevelProcess model may cover more than one perspective.
     * <p>Is used, e.g., to make sensible pre-selections when merging different simulation
     * models.
     */
    public enum Perspective {
        ...
        // one choice may be based on data while another one is based on probabilities
    ...
    public enum AttributeType {
        ...
        Nominal, // nominal attributes are enumerations of values, among which an initial value
    ...
    /**
     * An enumeration type for the way in which input data to a task is transformed
     * into output data for that task.
     * <br>
     * For example, a data attribute can be resampled or reused. As soon as there are
     * input and output data attributes for an activity, more complex transformation
     * types will become possible.
     */
    public enum TransformationType {
        Resample, // generate random value according to specification
        Reuse; // write back the same value as was read (unchanged but, e.g., logged)
    ...
    /**
     * Enumeration type for the different event types that the user can decide for
     * which some specific type of timing information needs to be selected from.
     */
