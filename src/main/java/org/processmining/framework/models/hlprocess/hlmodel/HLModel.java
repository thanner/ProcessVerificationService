...
/**
 * Allows to associate actual graph nodes in a control-flow process model
 * with high-level process information.
 * <br>
 * Task nodes can be mapped on HLActivities, and choice nodes
 * on HLChoices.
 * <br>
 * To provide this kind of mapping is the main task
 * of the interface and any implementing class.
 * <br>
 * The actual high-level information is encapsulated in the referenced
 * HLProcess object.
 *
 * @see HLProcess
 * @see HLActivity
...
public abstract class HLModel implements Cloneable {
    ...
    /**
     * the mapping from all choice nodes in the model to its high level choice ID
     */
     ...
     * Default constructor.
     *
     * @param aModel the control-flow model to which the high level information should be mapped
     */
     ...
     * <p>Deriving sub classes should override this method to establish an
     * appropriate default mapping and call this method in the very beginning.
     * <p>Should be called in the constructor of deriving sub classes and
     * will be used to reset the high-level model to default information.
     */
     ...
     *
     * @param process the HLProcess object containing the high-level information
     */
     ...
     * <p>Per default the plain process model is returned here.
     * If a diagnostic visualization is desired, implementing classes should return
     * a new object of type {@link HighLevelVisualization} every time this method is called.
     *
     * @param the perspectives that should be visualized for this model
     * @return the diagnostic visualization object for the current high level information
     */
     ...
     * <p>Sub classes can determine which activities they want
     * to hide. Per default all activities are visible.
     *
     * @return
     */
    ...
    /**
     * Retrieves the activity ID stored for the given vertex in the activity mapping.
     *
     * @param vertex the vertex for which the activity ID should be retrieved
    ...
    /**
     * Retrieves the choice ID stored for the given vertex in the choice mapping.
     *
     * @param vertex the vertex for which the choice ID should be retrieved
    ...
    /**
     * Returns the first ModelGraphVertex that is mapped to the given high level activity.
     *
     * @param actID the ID of the high level activity for which we want to have the corresponding
    ...
    /**
     * Returns the first ModelGraphVertex that is mapped to the given high level choice.
     *
     * @param choiceID the ID of the high level choice for which we want to have the corresponding
     *                 modelgraphVertex in the graph
    ...
    /**
     * Replaces the old vertex in by the given new value in the activity mapping.
     * If it does not exist, the method has no effect
     *
     * @param oldVertex the vertex to be replaced
     * @param newVertex the new vertex for the given activity
     */
    public void replaceModelGraphVertexForActivity(ModelGraphVertex oldVertex, ModelGraphVertex newVertex) {
     ...
     *
     * @param oldVertex the vertex to be replaced
     * @param newVertex the new vertex for the given choice
     */
    public void replaceModelGraphVertexForChoice(ModelGraphVertex oldVertex, ModelGraphVertex newVertex) {
    ...
    /**
     * Sets the corresponding highlevelactivity for a node of the process model that belongs
     * to the highlevelprocess.
     * <p>Note that if there was a previous HLActivity associated to this vertex, it will be
     * replaced in the high-level process by the new one.
     * <p>This has the side effect that if other task nodes in the process sharing the same
     * high-level activity would point to an HLActivity that is not anymore in the process.
     * They need to be updated as well.
     *
     * @param vertex   ModelGraphVertex a node in the process model that belongs to the highlevelprocess
     * @param activity HLActivity the highlevelactivity that has to be assigned to the node in
     *                 the process model
     */
    public void setActivity(ModelGraphVertex vertex, HLActivity activity) {
        HashMap<ModelGraphVertex, HLID> modifiedMapping = (HashMap<ModelGraphVertex, HLID>) vertexToHLActivityMapping.clone();
    ...
    /**
     * Sets the corresponding highlevelchoice for a node of the process model that belongs
     * to the highlevelprocess.
     * <p>Note that if there was a previous HLChoice associated to this vertex, it will be
     * replaced in the high-level process by the new one.
     * <p>This has the side effect that if other nodes in the process sharing the same
     * high-level choice would point to an HLChoice that is not anymore in the process.
     * They need to be updated as well.
     *
     * @param vertex ModelGraphVertex a node in the process model that belongs to the highlevelprocess
     * @param choice HLChoice the highlevelactivity that has to be assigned to the node in
     *               the process model.
     */
    public void setChoice(ModelGraphVertex vertex, HLChoice choice) {
        HashMap<ModelGraphVertex, HLID> modifiedMapping = (HashMap<ModelGraphVertex, HLID>) vertexToHLChoiceMapping.clone();
    ...
    /**
     * Returns the HLActivity object belonging to the given node.
     * <br>
     ...
     *
     * @param v ModelGraphVertex the vertex for which the corresponding simulation information object is to be found
    ...
    public HLActivity findActivity(ModelGraphVertex v) {
        ...
        // not found
        Message.add("Finding the high-level activity for model graph vertex " + v.getIdentifier() +
                " has failed.", Message.DEBUG);
        return null;
    }

    /**
     * Returns the HLChoice object belonging to the given node.
     * <br>
     * Note that the given modelgraphvertex only needs to be equal to the
     * modelgraphvertex of the belonging graph but not the same object.
     *
    ...
    public HLChoice findChoice(ModelGraphVertex v) {
        for (Entry<ModelGraphVertex, HLID> entry : vertexToHLChoiceMapping.entrySet()) {
            if (entry.getKey() != null && entry.getKey().equals(v)) {
                return hlProcess.getChoice(entry.getValue());
            }
        }
        // not found
        Message.add("Finding the high-level choice for model graph vertex " + v.getIdentifier() +
                " has failed.", Message.DEBUG);
        return null;
    }

    /**
     * Retrieves all nodes of the process model that could be mapped to elements of
     * the high-level process (such as activities or choices).
     *
     * @return the nodes of the process model that can be mapped to the high-level process
     * elements. The list can be empty
     */
    ...
    public List<ModelGraphVertex> getActivityNodes() {
        return new ArrayList<ModelGraphVertex>(vertexToHLActivityMapping.keySet());
    }
    ...
    /**
     * Makes a deep copy of the object, but does not clone the underlying process model.
     * <br>
     * Clones the attached high level process, and the high-level transition and
     * the high-level choice mappings.
     * <br>
     * The underlying Process model must not cloned as object identity of
     * graph nodes is needed to retrieve the related high-level objects after cloning.
     *
    ...
    public Object clone() {
        ...
        try {
            ...
            o.vertexToHLActivityMapping = (HashMap<ModelGraphVertex, HLID>) vertexToHLActivityMapping.clone();
            o.vertexToHLChoiceMapping = (HashMap<ModelGraphVertex, HLID>) vertexToHLChoiceMapping.clone();
            return o;
